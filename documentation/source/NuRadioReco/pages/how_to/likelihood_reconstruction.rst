...
The code consists of 4 core modules that handle different parts of the reconstruction:
  ...
  - :class:`LikelihoodCalculator <NuRadioReco.modules.likelihood_reconstruction.LikelihoodCalculator>`: Calculates the likelihood
  ...
  - :class:`ShowerSimulator <NuRadioReco.modules.likelihood_reconstruction.ShowerSimulator>`: Simulates the signal traces from a
...
An example script demonstrating how to use the core modules is available in :download:`run_shower_simulator.py <../../../../../NuRadioReco/examples/likelihood_reconstruction/run_shower_simulator.py>` and a script demonstrating the :class:`ShowerSimulator <NuRadioReco.modules.likelihood_reconstruction.ShowerSimulator>` is available here :download:`toy_signal_likelihood_fit_and_matched_filter.py <../../../../../NuRadioReco/examples/likelihood_reconstruction/toy_signal_likelihood_fit_and_matched_filter.py>`.


These modules are then combined in modules for specific reconstruction tasks with easy-to-use interfaces:

  - :class:`neutrinoLikelihoodReconstructor <NuRadioReco.modules.likelihood_reconstruction.neutrinoLikelihoodReconstructor>`: Reconstructs a neutrino
  ...
  - :class:`electricFieldLikelihoodReconstructor <NuRadioReco.modules.likelihood_reconstruction.electricFieldLikelihoodReconstructor>`: Reconstructs a pulsed
...
The :class:`neutrinoLikelihoodReconstructor <NuRadioReco.modules.likelihood_reconstruction.neutrinoLikelihoodReconstructor>` reconstructs the parameters of a shower from a neutrino signal given a set of measured traces.
...
A full example of how to run the :class:`neutrinoLikelihoodReconstructor <NuRadioReco.modules.likelihood_reconstruction.neutrinoLikelihoodReconstructor>` is shown in :download:`neutrino_signal_reconstruction.py <../../../../../NuRadioReco/examples/likelihood_reconstruction/neutrino_signal_reconstruction.py>`


Electric-field reconstruction
-----------------------------

The :class:`electricFieldLikelihoodReconstructor <NuRadioReco.modules.likelihood_reconstruction.electricFieldLikelihoodReconstructor>` assumes that the data traces are stored in a station object. The reconstruction module is then intialized with:
...
A full example of how to run the :class:`electricFieldLikelihoodReconstructor <NuRadioReco.modules.likelihood_reconstruction.electricFieldLikelihoodReconstructor>` is shown in :download:`electric_field_reconstruction.py <../../../../../NuRadioReco/examples/likelihood_reconstruction/electric_field_reconstruction.py>`
