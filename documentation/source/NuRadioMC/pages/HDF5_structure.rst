...
station triggered, with which amplitude, etc. The same approach works for ``shower_id``.
    ...
    .. csv-table:: HDF5 station items
            ...
            ``polarization`` | (``m_showers``, ``n_channels``, ``n_ray_tracing_solutions``, ``3``) | 3D coordinates of the polarization vector at the antenna in cartesian coordinates. (The receive vector was used to rotate from spherical/on-sky coordinates to cartesian coordinates).  
