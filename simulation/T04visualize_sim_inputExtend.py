...
#plt.hist(np.cos(zeniths / units.radian), bins = np.arange(-1, 1, 0.2))
plt.xlabel('zenith [deg]')
plt.subplot(1, 2, 2)
plt.hist(azimuths / units.deg, bins = np.arange(0, 361, 45))
plt.xlabel('azimuth [deg]')
plt.suptitle('incoming signal direction')
plt.savefig(os.path.join(plot_folder, 'simInputIncoming.pdf'), bbox_inches="tight")
plt.clf()

# plot direction distribution
fig, ax = plt.subplots(1, 1)
h = ax.hist2d(azimuths / units.deg, zeniths / units.deg, bins=[np.arange(0, 361, 5), np.arange(0, 181, 5)],
              cmap=plt.get_cmap('Blues'))
cb = plt.colorbar(h[3], ax=ax)
cb.set_label("number of events")
ax.set_aspect('equal')
