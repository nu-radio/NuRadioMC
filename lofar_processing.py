...
def get_sim_files(df: pd.DataFrame) -> dict:
...
def lofar_data_processing(df: pd.DataFrame, output_dir: Path, sim_files: dict, snr: float):
    ...
    for i, series in df.iterrows():
        ...
        rfi_filter.begin(reader=reader, rfi_cleaning_trace_length=65536)
        rfi_filter.run(evt)
        rfi_filter.end()
        ...
        try:
            ...
            axis_reco.run(
                ...
                n_samples=None,
                cross_section_size=500,
                cross_section_spacing=[20, 20, 20, 10, 10, 10],
            )
        ...
        if (i % 25 == 0 and i != 0) or (i == len(df) - 1):
            logger.info(f"Dumping events -- Last event {event_id}")
            with open(
