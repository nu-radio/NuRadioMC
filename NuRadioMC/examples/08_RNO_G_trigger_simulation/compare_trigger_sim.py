...
class mySimulation(simulation.simulation):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.highLowThreshold = highLowTrigger()
        self.rnogADCResponse = triggerBoardResponse.triggerBoardResponse()
        self.rnogADCResponse.begin(clock_offset=0.0, adc_output="counts")
        ...
        self.channel_generic_noise_adder.begin() #seed=secrets.randbits(128))
        self.powerTrigger = powerTrigger()
        self.envelopeTrigger = envelopeTrigger()
    ...
    def _detector_simulation_trigger(self, evt, station, det):
        ...
        vrms_after_gain, gains = self.rnogADCResponse.run(
            ...
            vrms=vrms_input_to_adc, digitize_trace=True, apply_adc_gain=True
        )
        ...
        threshold_high = {channel_id: int(np.rint(3.8 * vrms)) for channel_id, vrms in zip(self.deep_trigger_channels, vrms_after_gain)}
        threshold_low = {channel_id: int(-1 * np.rint(3.8 * vrms)) for channel_id, vrms in zip(self.deep_trigger_channels, vrms_after_gain)}
        ...
        self.highLowThreshold.run(
            ...
            trigger_name=f"high_low",
            **triggerBoardResponse.highLowTriggerKwargs
        )
        
        lvrms=0
        for i in deep_trigger_channels:
        ...
        self.powerTrigger.run(
            ...
            **triggerBoardResponse.powerTriggerKwargs
        )
        ...
        self.envelopeTrigger.run(
            ...
            **triggerBoardResponse.envelopeTriggerKwargs
        )
...
if __name__ == "__main__":
    ...
    def_data_dir = os.path.join(ABS_PATH_HERE, "data3")
    ...
    # Neutrino arguments
    parser.add_argument("--energy", '-e', default=19, type=float, help="Neutrino energy [eV]")
    ...
    parser.add_argument("--n_triggers", type=int, default=1e3, help="Number of total events to generate")

    args = parser.parse_args()
    kwargs = args.__dict__
    kwargs["energy"]=10**(kwargs["energy"])
    ...
    r = myrunner(args.n_cpus, task, output_path, max_runtime=3600 * 3, n_triggers_max=args.n_triggers, kwargs=kwargs)
    r.run()