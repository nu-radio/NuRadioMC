...
def generate_my_events(filename, n_events):
    ...
    attributes['start_event_id'] = 0    # define the fiducial simulation volume. Instead of specifying fiducial_rmin and fiducial_rmin one can also specify
    ...
    #For Some pulses, you have choice for the half width and frequency
    data_sets["half_of_pulse_width"]= 100*np.ones(n_events) ##This helps to shape the width of square and tone_burst pulse width in nano seconds 
    data_sets["freq"] = 0.3*np.ones(n_events)  # This will be frequency of a signal in MHz ( for cw and tone_burst model)
