...
int main(int argc, char **argv){
	...
	TH1D *hist = new TH1D("Histogram of Ray Tracing Times","Histogram of Ray Tracing Times",100,0,5e-4);
	TH1D *hist_0sol = new TH1D("0sol","0sol",100,0,5e-4);
	TH1D *hist_1sol=new TH1D("1sol","1sol",100,0,5e-4);
	TH1D *hist_2sol=new TH1D("2sol","2sol",100,0,5e-4);

	double average_time=0.;
	int tries=0;
	while(tries<100000){

		double x1[2] = {random->Uniform(-2000.,2000.),random->Uniform(-3000.,0.)};
		double x2[2] = {random->Uniform(-2000.,2000.),random->Uniform(-3000.,0.)};

		double this_start_time = getUnixTime();
			vector<vector<double> > solutions = find_solutions(x1,x2, n_ice, delta_n, z_0);
			for(int sol=0; sol<solutions.size(); sol++){
				// get_path_length(x1,x2,solutions[sol][1],n_ice,delta_n,z_0);
				// get_launch_angle(x1,solutions[sol][1],n_ice,delta_n,z_0);
				// get_receive_angle(x1,x2,solutions[sol][1],n_ice,delta_n,z_0);
			}
		double this_stop_time = getUnixTime();
		double this_difference = this_stop_time - this_start_time;

		hist->Fill(this_difference);
		if(solutions.size()>0){
			int num_solutions = solutions.size();
			if(num_solutions==1){
				hist_1sol->Fill(this_difference);
			}
			else if(num_solutions==2){
				hist_2sol->Fill(this_difference);		
			}
		}
		else{
			hist_0sol->Fill(this_difference);
		}
		average_time+=this_difference;
		tries++;
	}
	average_time/=double(tries);
	cout<<"Average difference is "<<average_time<<endl;
	...
	hist_0sol->Draw("same");
		hist_0sol->SetLineColor(kGreen);
		hist_0sol->SetLineWidth(3);
		// hist_0sol->SetLineStyle(9);		
	hist_1sol->Draw("same");
		hist_1sol->SetLineColor(kBlue);
		hist_1sol->SetLineWidth(3);
		// hist_1sol->SetLineStyle(3);	
	hist_2sol->Draw("same");
		hist_2sol->SetLineColor(kRed);
		hist_2sol->SetLineWidth(3);
		// hist_2sol->SetLineStyle(4);	
	hist->GetXaxis()->SetTitle("Time (s)");
	hist->GetYaxis()->SetTitle("Counts");

	TLegend *leg = new TLegend(0.48,0.75,0.68,0.9);
	leg->AddEntry(hist,"All Events","l");
	leg->AddEntry(hist_0sol,"No Solutions","l");
	leg->AddEntry(hist_1sol,"One Solution","l");
	leg->AddEntry(hist_2sol,"Two Solution","l");
	leg->Draw();
	c->SaveAs("hist_of_solve_times.png");
