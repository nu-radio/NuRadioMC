...
for i_param in range(n_output_parameters):
    ...
    ax[i_param].set_ylabel(
        parameter_labels[i_param] + " [" + parameter_unit_labels[i_param % 4] + "]"
    )
fig.tight_layout()
...
for i_param in range(n_output_parameters):
    ax[i_param].scatter(
        X_array[:, 0],
        results_c8[:, i_param] - results_analytic[:, i_param],
        color="C3",
        marker="o",
        alpha=1,
        s=10,
    ...
    ax[i_param].set_ylabel(
        "Delta "
        + parameter_labels[i_param]
        + " ["
        + parameter_unit_labels[i_param % 4]
        + "]"
    )
fig.tight_layout()
...
for i_param in range(n_output_parameters):
    ...
    ax[i_param].plot(
        x_det, z_det, ls="None", marker="*", color="k", markersize=10, label="Detector"
    )
    ax[i_param].axhline(0, color="k", ls="--", lw=1)
...
print("\nRaytracing took", t1 - t0, "s")
print("Tracer 1 took", t1_tot, "s")
print("Tracer 2 took", t2_tot, "s")
