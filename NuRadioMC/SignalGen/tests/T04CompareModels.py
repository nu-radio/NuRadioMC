...
for iTheta, theta in enumerate(thetas):
    ...
    ax[iTheta].plot(tt, trace, '-C0'.format(iTheta), label=r"$\Delta \Theta$ = {:.1f}".format((theta-theta_C)/units.deg))
    trace = ask.get_time_trace(E, theta, N, dt, shower_type, n_index, R, "Alvarez2000")
    trace = np.roll(trace, int(-1 * units.ns/dt))
    ax[iTheta].plot(tt, trace, '--C1'.format(iTheta), label=r"$\Delta \Theta$ = {:.1f}".format((theta-theta_C)/units.deg))
    ax[iTheta].set_title(r"$\Delta \Theta$ = {:.1f}".format((theta-theta_C)/units.deg))
