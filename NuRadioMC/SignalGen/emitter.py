...
def get_time_trace(amplitude, N, dt, model, full_output=False, **kwargs):
    ...
    if(model == 'delta_pulse'):  # this takes delta signal as input voltage
    ...
    elif(model == 'idl1' or model == 'hvsp1' or model == 'ARA02_calPulser' or model.startswith("rno_")):  # the idl1 & hvsp1 waveforms gemerated in KU Lab stored in hdf5 file, rno waveforms generated in chicago lab
        ...
        if model not in buffer_emitter_model:
            path = os.path.dirname(os.path.dirname(__file__))
            SPice_pulses = os.path.join(path,"examples/SignalProp/birefringence_examples/extra_files/SPice_pulses.xz")
