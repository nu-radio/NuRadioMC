...
#        n_events = 1000
        ...
        max_amp_ray_solution = np.zeros((n_events, n_antennas, 2)) * np.nan
...
#                 print("*", end='')
        ...
        #             print("{} {:.2f} {:.0f}".format(polarization_direction_onsky, np.linalg.norm(polarization_direction_onsky), np.arctan2(np.abs(polarization_direction_onsky[1]), np.abs(polarization_direction_onsky[2])) / units.deg))
            ...
            for sim_channel in station.get_sim_station().iter_channels():
                for iCh2, sim_channel2 in enumerate(sim_channel):
                    ...
                    max_amp_ray_solution[iE, channel_id, iCh2] = sim_channel2.get_parameter(
            ...
            if(triggered[iE] and (weights[iE] > minimum_weight_cut)):
                ...
                for channel in station.get_channels():
                    ...
                    maximum_amplitudes_envelope[iE, channel.get_id()] = channel.get_parameter(
                        chp.maximum_amplitude_envelope)
        ...
        fout['max_amp_ray_solution'] = max_amp_ray_solution[triggered]
