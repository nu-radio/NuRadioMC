...
import json
import mattak.Dataset as mattak
...
class stationLDAVariables:
    ...
    def __init__(self):
        self.logger = logging.getLogger('NuRadioReco.stationLDAVariables')
    ...
    def begin(self,coincidence_window_size=6,pad_length=500,channel_ids=[0,1,2,3]):
        ...
        coincidence_window_size : window size used for calculating the maximum peak to peak amplitude

        pad_length : padding length used for calculating the coherent sum

        channel_ids : channels for which to calculate the variables
    ...
    def run(self, event, station, detector, channel_id=0):
        ...
        """
        # print('channel:', station.get_channel(channel_id))
        # print('max_a=',self.max_a(event, station, detector))
        # print('avg_ch_snr=',self.avg_ch_snr(event, station, detector))
        # print('coherent_sum_step_by_step=',self.coherent_sum_step_by_step(event, station))
        # print('coherent_sum=',self.coherent_sum(event, station, station.get_channel(channel_id)))
        # print('coherent_snr=',self.coherent_snr(self.sum_chan))
        # print('impulsive_value=',self.impulsive_value(station.get_channel(channel_id).get_trace()))
        station.set_parameter('max_a', self.max_a(event, station, detector))
        station.set_parameter('avg_ch_snr', self.avg_ch_snr(event, station, detector))
        self.coherent_sum(event, station, station.get_channel(channel_id))
        station.set_parameter('coherent_snr', self.coherent_snr(event, station, detector))
        station.get_channel(channel_id)
        return
    def end(self):
        pass
