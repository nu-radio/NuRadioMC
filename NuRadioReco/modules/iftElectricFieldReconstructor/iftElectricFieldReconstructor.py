...
class IftElectricFieldReconstructor:
    ...
    def run(self, event, station, detector, grouped_channel_ids, efield_scaling, ray_type, use_sim=False, plot_title='', polarization=None):
        ...
        if self.__phase_slope == 'both' or self.__phase_slope == 'positive':
            ...
            self.__draw_priors(event, station, frequency_domain)
            if len(self.__used_grouped_channel_ids) > 1:
    ...
    def __prepare_traces(
        ...
        lss = ("-", "--", ":", "-.", (0, (3, 5, 1, 5, 1, 5)), (0, (3, 5, 1, 5, 1, 5, 1, 5)))
        i_channel = -1
        for i_channel_group, channel_group in enumerate(self.__used_grouped_channel_ids):
            c = colors[i_channel_group]
            for j_channel, channel_id in enumerate(channel_group):
                i_channel += 1
                ls = lss[j_channel]
                trace = self.__data_traces[i_channel]
                ax.plot(t, trace, c=f"C{i_channel_group}", alpha=0.7, label=f"ch {channel_id:>2}, gr {i_channel_group}", ls=ls, lw=3)
    ...
    def __get_likelihood_operator(
        ...
        correlated_fields_delta = [0]*n_groups
        i_channel = -1
        ...
        for i_channel_group, channel_group in enumerate(self.__used_grouped_channel_ids):
            ...
            if i_channel_group != 0:
                ...
                self.__spec_difference[i_channel_group] = (domain_flipper @ zero_padder.adjoint @ correlated_fields_delta[i_channel_group-1])
                self.__spec_difference[i_channel_group] = (NuRadioReco.modules
                                                                      .iftElectricFieldReconstructor
                                                                      .operators
                ...
                self.__spec_difference[i_channel_group] *= scaling_factor
            ...
            for n_channel, channel_id in enumerate(channel_group):
                ...
                print("get likelihood for ", i_channel, ": channel ", channel_id)
                ...
                if self.__polarization == 'theta':
                    efield_spec_operator_theta = ((filter_operator
                                                   @ ((mag_S_h + self.__spec_difference[i_channel_group])
                                                   * (1.j * phi_S_h).exp())))
    ...
    def __draw_priors(
        ...
        fig1.tight_layout()
        fig1.savefig(f'{self.__plot_folder}/{event.get_run_number()}_priors_{event.get_id()}.png')
        ...
        for i in range(n_priors):
            ...
            ax[0].plot(x_axis, efield_spec_sample.val)
        ...
        for i in range(n_groups-1):
            ...
            for i in range(n_priors):
                x = ift.from_random('normal', self.__spec_difference[i_group].domain)
                efield_spec_sample = self.__spec_difference[i_group].force(x)
    ...
    def __draw_priors_delta(
        ...
        for i in range(8):
            ...
            ax1_4.plot(times, channel_trace_sample.val / np.max(np.abs(channel_trace_sample.val)), c='C{}'.format(i), alpha=alpha)
            a = self.__delta_power_spectrum_operator.force(x).val
        ...
        ax1_4.set_title('Channel Trace')
        fig1.tight_layout()
        fig1.savefig(f'{self.__plot_folder}/{event.get_run_number()}_priors_delta_{event.get_id()}.png')
    ...
    def __draw_reconstruction(
        ...
        median = KL.position
        sampling_rate = station.get_channel(self.__used_channel_ids[0]).get_sampling_rate()
        fig1 = plt.figure(figsize=(16, (4.5 * n_channels)))
        #fig1 = plt.figure(figsize=((8*3), (5 * 0.33 * n_channels)))
        #plotting
        dummy_fig = plt.figure(figsize=(16, 4))
        dummy_fig = fig1
        ...
        for i_channel, channel_id in enumerate(self.__used_channel_ids):
            ...
            if self.__polarization == 'pol':
                #n_columns = 2
                #n_rows = int(np.ceil(n_channels/n_columns))

                step_size = 3 # = 1
                n_rows = (n_channels)
                n_columns = 3

                ax1_1 = fig1.add_subplot(n_rows, n_columns, step_size * i_channel + 1)
                ax1_2 = dummy_fig.add_subplot(n_channels, 3, 3 * i_channel + 2)
                ax1_3 = dummy_fig.add_subplot(n_channels, 3, 3 * i_channel + 3, sharey=ax1_2)
                ...
                ax1_2 = dummy_fig.add_subplot(n_channels, 2, 2 * i_channel + 2)
            ...
            for sample in KL.samples:
                ...
                for i_pol, efield_stat_calculator in enumerate(efield_stat_calculators):
                    ...
                    ax1_1.plot(freqs / units.MHz, amp_trace * self.__scaling_factor / units.mV, c='k', alpha=0.2)
                    #ICHANCGED THIS
                    ax1_1.plot(freqs / units.MHz, amp_trace * self.__scaling_factor / units.mV, c='k', alpha=0)
        ...
        for i_group in range(n_groups-1):
            ...
            x = ift.from_random('normal', self.__spec_difference[i_group+1].domain)
            efield_spec_sample = self.__spec_difference[i_group+1].force(x)
        ...
        median = KL.position
        sampling_rate = station.get_channel(self.__used_channel_ids[0]).get_sampling_rate()
        l = len(self.__used_grouped_channel_ids)
        n_subfigs = int((l * (l-1))/2)
        n_rows = 1 + int(np.ceil(n_subfigs/2))

        fig1 = plt.figure(figsize=(16, n_rows*4))

        ax1_1 = fig1.add_subplot(n_rows,2,(1,2))
        ax = []
        i = -1
        for j in range(len(self.__used_grouped_channel_ids)):
            for k in range(j):
                i += 1
                ax.append(fig1.add_subplot(n_rows,2,3+i))
        ...
        for i_channel_group, channel_group in enumerate(self.__used_grouped_channel_ids):
            ...
            for n_channel, channel_id in enumerate(channel_group):
                ...
                if first_channel:
                    ...
                    group_traces.append(amp_trace_stat_calculator.mean)
                    first_channel=False
        ...
        for j in range(n_groups):
            for k in np.arange(j+1,n_groups):
                i += 1
                trace = group_traces[j] - group_traces[k]
                ax[i].plot(freqs / units.MHz, trace * self.__scaling_factor / units.mV, c=colors[0], linewidth=3, alpha=1, label=i_channel_group)
                ax[i].set_title(f"Difference group {j} - {k}")
                ax[i].grid()
                ax[i].set_xlabel('f [MHz]', fontsize=fontsize)
                ax[i].set_ylabel('channel voltage [mV/GHz]', fontsize=fontsize)
                ax[i].tick_params(axis='both', labelsize=fontsize)
                ax[i].set_xlim([self.__passband[0] / units.MHz - 50, self.__passband[1] / units.MHz + 50])
                if sim_efield_max is not None:
                    ax[i].set_ylim([0, 1.2 * sim_efield_max / (units.mV / units.m / units.GHz)])
