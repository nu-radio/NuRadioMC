...
class PhasedArrayBase():
    ...
    def calculate_time_delays(
        ...
        delays = -(ant_z - ref_z) / cspeed * ref_index * np.sin(phasing_angles[:, None]) - cable_delays
        delays = delays - np.max(delays, axis=1)[:, None]
        beam_rolls = np.array(np.round(delays / time_step)).astype(int)

        return np.array(beam_rolls)
