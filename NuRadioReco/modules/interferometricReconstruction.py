...
class InterferometricReco:
            ...
            if not os.path.exists(path):
                ...
                try:
                    with np.load(path) as npz:
                        tt_map[ch] = {
                                "r_range": npz["r_range_vals"].copy(),
                                "z_range": npz["z_range_vals"].copy(),
                                "data": npz["data"].copy()}
    ...
    class CorrScoreProvider:
        ...
        def process_csp(self, pair, channel_times, channel_sigvals, upsample):
            ...
            corrs_num = signal.correlate(sig_a_rs_norm, sig_b_rs_norm, mode = "full")
            
            """ 
            np.save(f"/data/condor_shared/users/avijai/RNO_reco/rno_dep/source/NuRadioMC/NuRadioReco/examples/RNOG/interferometric_reconstruction/corr_arrays_0318_1000/{pair}_norm.npy", normfact)
            np.save(f"/data/condor_shared/users/avijai/RNO_reco/rno_dep/source/NuRadioMC/NuRadioReco/examples/RNOG/interferometric_reconstruction/corr_arrays_0318_1000/{pair}_corrs_num.npy", corrs_num)
            np.save(f"/data/condor_shared/users/avijai/RNO_reco/rno_dep/source/NuRadioMC/NuRadioReco/examples/RNOG/interferometric_reconstruction/corr_arrays_0318_1000/{pair}_tvals.npy", tvals)
            """
        ...
        def __init__(self, channel_sigvals, channel_times, channel_pairs_to_include, cores, upsample = 10):
            ...
            for pair, corrs, tvals in results:
                ...
                """
                plt.figure()
                plt.scatter(tvals, corrs)
                plt.xlabel("time")
                plt.ylabel("correlation")
                plt.title(f"Correlation Function {pair} Padding")
                plt.savefig(f"/data/condor_shared/users/avijai/RNO_reco/rno_dep/source/NuRadioMC/NuRadioReco/examples/RNOG/interferometric_reconstruction/corr_func_0318/corr_func_{pair}_pad.png")
                plt.close()
                """
    ...
    def nearest_index(self, grid, values):
        ...
        #idx_clip = np.clip(idx, 1, len(grid) - 1)
        #same = np.array_equal(idx, idx_clip)
    ...
    def get_travel_time(self, ttcs, ch, coord):
        ...
        #r = np.linspace(0, 2100, 30000)
        #z = np.linspace(-2100, 600, 30000)
        ...
        interp = RegularGridInterpolator((r, z),times,method="linear",bounds_error=True)
        values = interp(coord)
        ...
        r_idx = self.nearest_index(r, r_coord)
        z_idx = self.nearest_index(z, z_coord)

        values = times[r_idx, z_idx]
        """
        """
        r_idx = np.searchsorted(r, r_coord)
        z_idx = np.searchsorted(z, z_coord)
        print(np.all(np.diff(z) > 0))

        # clamp to valid range
        r_idx = np.clip(r_idx, 0, len(r) - 1)
        z_idx = np.clip(z_idx, 0, len(z) - 1)
    ...
    def process_pair(self, pair, travel_times, csp):
        ...
        """
        outdir = "/data/condor_shared/users/avijai/RNO_reco/rno_dep/source/NuRadioMC/NuRadioReco/examples/RNOG/interferometric_reconstruction/corrvals_tvals_0222"
        np.save(f"{outdir}/corrvals_{ch_a}_{ch_b}.npy", corrvals)
        np.save(f"{outdir}/tvals_{ch_a}_{ch_b}.npy", tvals)
        """
        #map delay times to correlation scores 
        score = np.interp(t_ab, tvals, corrvals, left=0, right=0)
        
        #outdir = "/data/condor_shared/users/avijai/RNO_reco/rno_dep/source/NuRadioMC/NuRadioReco/examples/RNOG/interferometric_reconstruction/corr_vs_delay_plots_ior1"
        #outdir = "/data/condor_shared/users/avijai/RNO_reco/rno_dep/source/NuRadioMC/NuRadioReco/examples/RNOG/interferometric_reconstruction/corr_vs_delay_ior1"

        #corr = np.load(f"{outdir}/corr_{ch_a}_{ch_b}.npy")
    ...
    def calc_corr_score(self,channel_signals, channel_times, pts, ttcs, origin, channel_pairs_to_include, channel_positions,csp, cores):
        ...
        #x = np.array([-401.9339267, 531.50847952, -924.78185138])
        
        """ 
        #x = np.array([808.68628432, 1183.47409399, -1308.52338618])
        x = np.array([2439.33624356, -1746.32147409, -2253.01204819])
        pts_vert = x.reshape(1, 3)
        print(pts_vert)
        tt_vert = {ch: self.get_travel_time(ttcs, ch, (self.to_antenna_rz_coordinates(pts_vert, channel_positions[ch]))) for ch in channels}
        print(tt_vert)
        """
    ...
    def build_interferometric_map_ang(self, channel_signals, channel_times, channel_pairs_to_include, channel_positions,
        ...
        *same as function interferometric_reco_ang
        ...
        """
        e_vert = np.array([-0.7020318778971605])
        az_vert = np.array([0.9713559458174563])
        ee_vert, aa_vert = np.meshgrid(e_vert, az_vert)
        pts2 = self.ang_to_cart(ee_vert.flatten(), aa_vert.flatten(), radius = 1877.30553208518, origin_xyz = origin_xyz)
        print(pts2)
        """
    ...
    def build_interferometric_map_rz(self, channel_signals, channel_times, channel_pairs_to_include, channel_positions,
        ...
        *same as function interferometric_reco_rz
        ...
        """    
        r_corr = [493.9759036144579]
        r2_corr = [2000.0]
        r3_corr = [3000.0]
        z_corr = [-421.68674698795166] 
        z2_corr = [-1518.0722891566265]
        z3_corr = [-2253.012048192771]

        zz_corr,rr_corr, = np.meshgrid(z_corr, r_corr)
        zz2_corr,rr2_corr, = np.meshgrid(z2_corr, r2_corr)
        zz3_corr,rr3_corr, = np.meshgrid(z3_corr, r3_corr)

        pts1 = self.rz_to_cart(zz_corr.flatten(), rr_corr.flatten(), azimuth = azimuth, origin_xyz = origin_xyz)
        pts2 = self.rz_to_cart(zz2_corr.flatten(), rr2_corr.flatten(), azimuth = azimuth, origin_xyz = origin_xyz)
        pts3 = self.rz_to_cart(zz3_corr.flatten(), rr3_corr.flatten(), azimuth = azimuth, origin_xyz = origin_xyz)
    ...
    def run(self, event, station, channels_to_include, azimuth_range, elevation_range, radius, z_range, r_range, num_pts_r, num_pts_z, num_pts_elev, num_pts_az, output_path, cores = 2, plotting = True, run_no = None, return_reco = False, return_score = False, return_delays = False, return_maps = False):
        ...
        if np.all(np.isnan(reco_rz["map"])):
            ...
            """ 
            max_corr_per_r = np.nanmax(mapdata, axis=1)
            z_indices = np.nanargmax(mapdata, axis=1)
            r_targets = [500, 2000, 3000]
            r_vals = reco_rz["r"]
            z_vals = reco_rz["z"]
            for r_target in r_targets:
                r_index = np.argmin(np.abs(r_vals - r_target))
                z_at_max = z_vals[z_indices[r_index]]
                max_corr = max_corr_per_r[r_index]

                print(f"r = {r_vals[r_index]}")
                print("   z at max corr:", z_at_max)
                print("   max corr:", max_corr)
            
            plt.figure()
            plt.plot(r_vals, max_corr_per_r)
            plt.xlabel("r")
            plt.ylabel("Max correlation (over z)")
            plt.title("Max correlation vs r")
            plt.savefig(f"/data/condor_shared/users/avijai/RNO_reco/rno_dep/source/NuRadioMC/NuRadioReco/examples/RNOG/interferometric_reconstruction/plots_0220_nu_gsrec_gs/maxcorr_r_{event.get_id()}.png")
            plt.close()
            """
