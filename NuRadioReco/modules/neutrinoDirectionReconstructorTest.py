#from NuRadioReco.framework.parameters import directionReconstructionParameters as drp
...
class neutrinoDirectionReconstructor:
    ...
    def run(self, event, shower_ids, station, det,
            use_channels=[6, 14], filenumber = 1, debugplots_path = None, template = False, sim_vertex = True, Vrms = 0.0114, only_simulation = False, ch_Vpol = 6, ch_Hpol = 13, full_station = True):
        ...
        only_simulation = True# True#False# if True, no fit is performed, but only the script is tested with the simulated values
        ...
        if 1:
            ...
            if only_simulation:
                ...
                zenith_start =  simulated_zenith - np.deg2rad(4)
                zenith_end =simulated_zenith +  np.deg2rad(4)
                azimuth_start =simulated_azimuth - np.deg2rad(4)
                azimuth_end = simulated_azimuth + np.deg2rad(4)
    ...
    def minimizer(self, params, vertex_x, vertex_y, vertex_z, minimize = True, timing_k = False, first_iter = False, banana = False,  direction = [0, 0], ch_Vpol = 6, ch_Hpol = 13, full_station = True):
            ...
            print("input direction", [np.rad2deg(zenith), np.rad2deg(azimuth)])
            ...
            #print(stop)
            traces, timing, launch_vector, viewingangles, raytypes, pol = self._simulation.simulation(self._det, self._station, vertex_x, vertex_y, vertex_z, zenith, azimuth, energy, self._use_channels, first_iter = first_iter)
            chi2 = 0
            all_chi2 = []
            over_reconstructed = []
            extra_channel = 0
            ...
            for iS in raytypes[ch_Vpol]:
                #print("raytypes[iS]", raytypes[3][iS])
                #print("self._station[stnp.raytype]", self._station[stnp.raytype])
           ...
           # print("raytype selected direction fit", ['direct', 'refracted', 'reflected'].index(self._station[stnp.raytype]) + 1)
           # print("selected iS", solution_number)     
...
#                        print("i trace",i_trace) 
                    ...
                    #        print("delta T", delta_T)
                            ...
                            #    print("YES")
                             ...
                             #   print("channel id {}, i trace {}".format(channel.get_id(), i_trace))
                               # print("dk {} delta T {}".format( dk, delta_T))
          ...
          #                              library_channels[6] = [6, 13]
                                    ...
                                    chi2_dt1 = np.sum((np.roll(rec_trace1, math.ceil(-1*dt1)) - data_trace_timing_1)**2 / ((sigma)**2))/len(rec_trace1)
                                    dt2 = np.argmax(corr) - (len(corr)/2) 
                                    chi2_dt2 = np.sum((np.roll(rec_trace1, math.ceil(-1*dt2)) - data_trace_timing_1)**2 / ((sigma)**2))/len(rec_trace1)
            ...
            #                        print("channel id", channel.get_id())    
                                    #print("dt", dt)
             #                       print("delta t ofst", delta_toffset)
             #                       print("DK", dk)
                                    ...
                                    if channel.get_id() == ch_Hpol:
                                       if (((max(channel.get_trace()) - min(channel.get_trace())) / (2*sigma)) < 4):#
            ...
            for i_ch in self._use_channels:
                print("ich", i_ch)
                print("dict", dict_dt[i_ch])
                dict_dt[i_ch][0] = dict_dt[ch_Vpol][0]
                dict_dt[i_ch][1] = dict_dt[ch_Vpol][0]
                #dict_dt[i_ch][1] = dict_dt[6][1]
            ...
            #dict_dt[14] = dict_dt[6]
            #dict_dt[5] = dict_dt[6]
            dof = 0
            for channel in self._station.iter_channels():
                #print("channel id ###### ", channel.get_id())
                if channel.get_id() in self._use_channels:
                   # print("channel id", self._use_channels)              
       	            chi2s = np.zeros(2)
                    echannel = np.zeros(2)
                    dof_channel = 0
                    rec_traces[channel.get_id()] = {}
                    ...
                    if traces[channel.get_id()]:
                        #print("channel id for traces", traces[channel.get_id()])
                        for i_trace, key in enumerate(traces[channel.get_id()]): ## iterate over ray type solutions
                           # print("channel id for traces", channel.get_id())
                            rec_trace_i = traces[channel.get_id()][key]
                            rec_trace = rec_trace_i
                            ...
                            if ((dk > 300)&(dk < len(np.copy(data_trace)) - 500)):
                                data_trace_timing = np.copy(data_trace) ## cut data around timing
                                ## DETERMIINE PULSE REGION DUE TO REFERENCE TIMING 
                                
                               # print("channel id dk", channel.get_id())
                                data_timing_timing = np.copy(channel.get_times())#np.arange(0, len(channel.get_trace()), 1)#
                                dk_1 = data_timing_timing[dk]
                                data_timing_timing = data_timing_timing[dk - 300 : dk + 500]
                                data_trace_timing = data_trace_timing[dk -300 : dk + 500]
                                data_trace_timing_1 = np.copy(data_trace_timing)
                                data_trace_timing_1[data_timing_timing < (dk_1 - 150)] = 0
                                data_trace_timing_1[data_timing_timing > (dk_1 + 350)] = 0
                                ...
                                rec_trace1 = rec_trace1[150 : 650]
                                data_trace_timing = data_trace_timing[150:  650]
                                data_timing_timing = data_timing_timing[150:  650]
                                delta_k.append(int(k_ref + delta_toffset + dt )) ## for overlapping pulses this does not work
                                ks[channel.get_id()] = delta_k
                                #print("data traces", data_traces[5][i_trace])
                            ...
                            # if vertex is wrong reconstructed, than it can be that data_timing_timing does not exist. In that case, set to zero.
                                ...
                                model_sys = 0
                                if  ((channel.get_id() == ch_Vpol) and (i_trace == trace_ref)):
                                    if 1:#
                                        trace_trig = i_trace
                                        echannel[i_trace] = 0
                                ...
                                elif ((channel.get_id() == ch_Hpol) and (i_trace == trace_ref)):
                                    if  1:
                                        echannel[i_trace] = 0
                                        mask_start = 0
                                        mask_end = 400
                                        ...
                                        chi2s[i_trace] = np.sum((rec_trace1[mask_start:mask_end] - data_trace_timing[mask_start:mask_end])**2 / ((sigma)**2))#/len(rec_trace1[mask_start:mask_end])
                                        data_tmp = fft.time2freq(data_trace_timing, self._sampling_rate) #* pol_filt
                                        ...
                                        if i_trace == trace_ref:
                                            Hpol_ref = np.sum((rec_trace1[mask_start:mask_end] - data_trace_timing[mask_start:mask_end])**2 / ((sigma)**2))/len(rec_trace1[mask_start:mask_end])
                                        reduced_chi2_Hpol += np.sum((rec_trace1[mask_start:mask_end] - data_trace_timing[mask_start:mask_end])**2 / ((sigma)**2))/len(rec_trace1[mask_start:mask_end])
                                        add = True
                                        dof_channel += 1
                                    
                                elif (SNR > 3.5) and (full_station):#and (i_trace ==trace_ref)):
                                    mask_start = 0
                                    echannel[i_trace] = 1
                                    mask_end = 400
              ...
              #                      print("over reconstructed", abs(max(rec_trace1[mask_start:mask_end]) - min(rec_trace1[mask_start:mask_end]))/(2*sigma))
                                    #chi2s[i_trace] = np.sum((rec_trace1 - np.zeros(len(rec_trace1)))**2 / (sigma)**2)
                                    if abs(max(rec_trace1[mask_start:mask_end]) - min(rec_trace1[mask_start:mask_end]))/(2*sigma) > 3.5:
                                     #   print("over reconstructed", abs(max(rec_trace1[mask_start:mask_end]) - min(rec_trace1[mask_start:mask_end]))/(2*sigma))
                                        chi2s[i_trace] = np.inf
                                        #if not minimize: 
                                        #    print('dat traces', channel.get_id())
                           ...
                           #                 data_timing[channel.get_id()][i_trace] = np.zeros(400)
                                        ...
                                        #else: 
                                        #    over_reconstructed.append(channel.get_id())
                                        #return np.inf
                        ...
                        rec_traces[channel.get_id()][0] = np.zeros(400)
                        data_traces[channel.get_id()][0] = np.zeros(400)
                        data_timing[channel.get_id()][0] = np.zeros(400)
                        rec_traces[channel.get_id()][1] = np.zeros(400)
                        data_traces[channel.get_id()][1] = np.zeros(400)
                        data_timing[channel.get_id()][1] = np.zeros(400)
        ...
        #                    print("data_timing[channel.get_id()][1]", data_timing[channel.get_id()][1])
                            ### overlapping pulses
    #                        print("over lapping pulses, not included in fit channel id {}", channel.get_id())
