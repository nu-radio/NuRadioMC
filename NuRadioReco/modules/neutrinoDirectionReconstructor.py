    ...
    def run(self, event, shower_ids, station, det,
        ...
        if 1:
           ...
           # viewing_end = np.deg2rad(cherenkov) + np.deg2rad(15)
            ...
            elif brute_force:# restricted_input:
...
#popt [ -0.7390428  113.74132012]
            ...
            elif brute_force:#not restricted_input:
                rotation_matrix = hp.get_rotation(sig_dir, np.array([0, 0,1]))
   ...
   # def minimizer_single_pulse(self, params, vertex_x, vertex_y, vertex_z, minimize = True, ch_Vpol = 6):
        #if len(params) == 2:
    ...
    def minimizer(self, params, vertex_x, vertex_y, vertex_z, minimize = True, timing_k = False, first_iter = False, banana = False,  direction = [0, 0], ch_Vpol = 6, ch_Hpol = 13, full_station = True, single_pulse = False, channels_step = [3], fixed_timing = False):
            ...
            if banana:
             #   print("banana")
                ...
                #    if np.rad2deg(cherenkov_angle) > 65:
                #        return np.inf
                #    if np.rad2deg(cherenkov_angle) < 40:
                #        return np.inf
              #      print("params", params)
                if len(params) == 2:
                ...
                if np.rad2deg(zenith) < 20: 
                    return np.inf
                #if np.rad2deg(azimuth) > (np.rad2deg(self._vertex_azimuth) + 50):
                #    return np.inf
                #if np.rad2deg(azimuth) < (np.rad2deg(self._vertex_azimuth) - 50):
                #    return np.inf
           ...
           # print("parameters zen {} az {} energy {}".format(np.rad2deg(zenith), np.rad2deg(azimuth), energy))
            ...
            #print("viewingangles", viewingangles)
            #if np.rad2deg(v) > 80:
               # return np.inf
            chi2 = 0
            all_chi2 = []
...
#                        print("i trace",i_trace) 
                    ...
                    if traces[channel.get_id()]:
                       ...
                       #         print("SNR single pulse full station channelstep", [SNR, single_pulse, full_station, channels_step])
                              ...
                              #          print("Hpol", channel.get_id())
                                    ...
                                    add = True
                            #        print("SNR > 3.5, channel {}, SNR {}".format( channel.get_id(), SNR))
                                    chi2s[i_trace] = np.sum((rec_trace1[mask_start:mask_end] - data_trace_timing[mask_start:mask_end])**2 / ((sigma)**2))#/len(rec_trace1[mask_start:mask_end])
                                    dof_channel += 1
                #                    extra_channel += 1
                             ...
                             #       print("else")
                                    ...
                                    mask_end = 400
                                    echannel[i_trace] = 0
                                   # print("over reconstructed channel", channel.get_id())
                                  #  print("over reconstructed SNR", abs(max(rec_trace1[mask_start:mask_end]) - min(rec_trace1[mask_start:mask_end]))/(2*sigma))#abs(max(rec_trace1[mask_start:mask_end]) - min(rec_trace1[mask_start:mask_end]))/(2*sigma))
                                    #chi2s[i_trace] = np.sum((rec_trace1 - np.zeros(len(rec_trace1)))**2 / (sigma)**2)
                                    if abs(max(rec_trace1[mask_start:mask_end]) - min(rec_trace1[mask_start:mask_end]))/(2*sigma) > 4.0:
        ...
        #                    print("data_timing[channel.get_id()][1]", data_timing[channel.get_id()][1])
            ...
            #print("CHI2 {}", chi2)# dof {}, chi2/dof ".format(chi2, dof))#, chi2/dof))
