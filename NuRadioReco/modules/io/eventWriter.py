...
def get_header(evt):
    ...
    for iS, station in enumerate(evt.get_stations()):
        ...
        header['stations'][station.get_id(
        )][stnp.station_time] = station.get_station_time_dict()

        if station.has_sim_station():
            header['stations'][station.get_id()]['sim_station'] = {}
            header['stations'][station.get_id()]['sim_station'] = station.get_sim_station(
            ).get_parameters().copy()
...
class eventWriter:
    ...
    def __write_fout_header(self):
        if self.__number_of_files > 1:
            self.__fout = open("{}_part{:02d}.nur".format(
                self.__filename, self.__number_of_files), 'wb')
        else:
    ...
    def begin(self, filename, max_file_size=1024, check_for_duplicates=False, events_per_file=None, log_level=logging.WARNING):
        ...
        if filename.endswith('.ari'):
            logger.warning(
                'The file ending .ari for NuRadioReco files is deprecated. Please use .nur instead.')
        ...
        # Remember which event IDs are already in file to catch duplicates
        self.__event_ids_and_runs = []
        # Remember if we still have to write the current file header
    ...
    def run(self, evt, det=None, mode=None):
        ...
        if det is not None:
            # returns None if detector is already saved
            detector_dict = self.__get_detector_dict(evt, det)
            if detector_dict is not None:
                detector_bytearray = self.__get_detector_bytearray(
                    detector_dict)
            ...
            if isinstance(det, generic_detector.GenericDetector):
                changes_bytearray = self.__get_detector_changes_byte_array(
                    evt, det)
        ...
        if self.__current_file_size > self.__max_file_size or self.__events_in_current_file == self.__events_per_file:
            logger.info(
                "current output file exceeds max file size -> closing current output file and opening new one")
            self.__current_file_size = 0
            self.__fout.close()
            self.__number_of_files += 1
        ...
        for station in event.get_stations():
            ...
            for channel in station.iter_channels():
                if not self.__is_channel_already_in_file(station.get_id(), channel.get_id(), station.get_station_time()):
                    if not is_generic_detector:
                        channel_description = det.get_channel(
                        ...
                        channel_description = det.get_raw_channel(
                            station.get_id(), channel.get_id())
                        self.__stored_channels.append({
                            'station_id': station.get_id(),
                            'channel_id': channel.get_id()
            ...
            if is_generic_detector:
                for reference_station_id in det.get_reference_station_ids():
                    if not self.__is_station_already_in_file(reference_station_id, None):
                        station_description = det.get_raw_station(
                            reference_station_id)
                        self.__stored_stations.append({
                            'station_id': reference_station_id
                        })
                        det_dict['stations'][str(
                            i_station)] = station_description
                        i_station += 1
                        for channel_id in det.get_channel_ids(reference_station_id):
                            if not self.__is_channel_already_in_file(reference_station_id, channel_id, None):
                                channel_description = det.get_raw_channel(
                                    reference_station_id, channel_id)
                                det_dict['channels'][str(
                                    i_channel)] = channel_description
                                self.__stored_channels.append({
                                    'station_id': reference_station_id,
    ...
    def __get_detector_changes_byte_array(self, event, det):
        changes = det.get_station_properties_for_event(
    ...
    def __check_for_duplicate_ids(self, run_number, event_id):
        ...
        if self.__check_for_duplicates:
            if [run_number, event_id] in self.__event_ids_and_runs:
                raise ValueError("An event with ID {} and run number {} already exists in the "
                                 "file\nif you don't want unique event ids enforced you can "
                                 "turn it of by passing `check_for_duplicates=True` to the "
                                 "begin method.".format(event_id, run_number))
        return
    ...
    def end(self):
        if self.__fout is not None:
            ...
            logger.warning(
                f"file {self.__filename} does not exist and won't be closed.")
