...
class triggerSimulator:
    ...
    def run(self, evt, station, det, threshold, coinc_window=60*units.ns, number_coincidences=1, triggered_channels=[13, 16, 19], temp=250*units.kelvin, Vbias=2*units.volt, trigger_name='rnog_surface_trigger'):
        ...
        for channel in station.iter_channels():
            ...
            mask = frequencies > 0
            b, a = scipy.signal.cheby1(3, 5, [80 * units.MHz, 180 * units.MHz], 'bandpass', analog=True)  # Numerator (b) and denominator (a) polynomials of the IIR filter
            #b, a = scipy.signal.butter(10, [80 * units.MHz, 180 * units.MHz], 'bandpass', analog=True)  # Numerator (b) and denominator (a) polynomials of the IIR filter
