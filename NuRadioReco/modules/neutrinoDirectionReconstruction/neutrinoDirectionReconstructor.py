                    ...
                    if traces[channel_id]:
                        for i_trace, key in enumerate(traces[channel_id]): ## iterate over ray type solutions
                            ...
                            if 1:#
                                ...
                                if (single_pulse):
                                ...
                                elif ((channel_id in self._PA_cluster_channels) and (i_trace == trace_ref)): #PA_cluster_channels contains all channels that are definitely included in the fit and for which the timings are fixed.
                                    ...
                                    dof_channel += 1
                                    echannel[i_trace] = 1
                                elif ((channel_id in self._use_channels) and (full_station) and (SNR > 3.5)):
                                    chi2s[i_trace] = np.sum((rec_trace - data_trace_timing)**2 / ((Vrms+model_sys*abs(data_trace_timing))**2))
                                    dof_channel += 1
