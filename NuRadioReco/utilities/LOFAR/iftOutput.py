...
PLOT_PARAMS = {
    ...
    "fg_edge": "#cc8b00",
    "old_color": "olivedrab",
    "cmap_fl_map": "plasma_r",
    "cmap_tm_map": "viridis",
    "cmap_fl_res": "inferno_r",
    "cmap_tm_res": "YlGn_r",
    "cmap_fl_cf_mean": "cividis_r",
    "cmap_fl_cf_std": "cividis_r",
    "cmap_tm_cf_mean": "GnBu_r",
    "cmap_tm_cf_std": "bone",
    "scatter_size": 40,
}
    ...
    try:
        ...
        if mc_truth is not None and np.isfinite(mc_truth):
            ...
            ax.axvline(
                ...
                lw=2.5,
                zorder=12,
                label=("MC truth: " + mc_truth_fmt) % mc_truth,
            )
...
def _plot_2d_contour_fancy(
    ax, x_data, y_data, old_point=None, x_label="", y_label="", title="",
    reference_label="LORA", truth_point=None
):
    ...
    if len(x_data) < 5:
        ...
        try:
            ...
            ax.contour(
                ...
                Y,
                Z,
                levels=sorted([level_3sig, level_2sig, level_1sig]),
                colors=LOFAR_GREEN,
                linewidths=[0.8, 1.2, 1.8],
                alpha=0.9,
            )
        except Exception:
    ...
    if old_point is not None:
        ax.scatter(
            *old_point,
            marker="*",
            s=150,
            c=PLOT_PARAMS["old_color"],
            zorder=10,
            label=reference_label,
        )
    if truth_point is not None and all(
            v is not None and np.isfinite(v) for v in truth_point):
        ax.scatter(
            *truth_point,
            marker="*",
            s=220,
...
def generate_reco_plot(
    ...
    direction_reference_label="Radio",
    ...
    ref_params : dict
        LORA reference: zenith/azimuth (rad), core ([x,y] m), and energy (eV).
    ...
    mc_truth : dict or None
        CoREAS truth for a simulated event: ``xmax_gpcm2``, ``energy_ev``,
        ``core_x_m``, ``core_y_m``, ``zenith_rad``, ``azimuth_rad``. Each
        posterior panel then carries the true value, in black.
    rejection_reason : str or None
        If the reconstruction failed its quality cuts, why. The figure is then
        stamped REJECTED and carries the reason. 
    """
    ...
    cmp_zenith_deg = (
        np.rad2deg(att_truth["zenith"])
        if (att_truth and "zenith" in att_truth)
        else old_zenith_deg
    )
    cmp_azimuth_deg = (
        (np.rad2deg(att_truth["azimuth"]) % 360.0)
        if (att_truth and "azimuth" in att_truth)
        else old_azimuth_deg
    )
    ...
    extent = [
        model_min_x,
        model_min_x + model_extent_val,
        model_min_y,
        model_min_y + model_extent_val,
    ]
    ...
    for s in samples:
        if grid_model is not None:
            ...
            try:
                correction = float(
                    np.asarray(signal_response.get_energy_correction_factor(s)).squeeze()
                )
            except Exception:
                correction = 1.0
            if not np.isfinite(correction) or correction <= 0:
                correction = 1.0
            fl_acc += np.asarray(res[0]).flatten() / correction
            tm_acc += np.asarray(res[1]).flatten()
        try:
            nm_acc += float(np.mean(np.asarray(signal_response.noise_floor(s))))
        except Exception:
            nm_acc += noise_mean
    ...
    sigma_fl = np.sqrt(
        _noise_sigma**2 + (fluences * _FLUENCE_RELATIVE_SYSTEMATIC_ERROR) ** 2
    )
    ...
    if res_fl.size > 0 and not np.all(np.isnan(res_fl)):
        vm_fl = max(
            abs(np.nanpercentile(res_fl, 1)), abs(np.nanpercentile(res_fl, 99)), 1.0
        )
    else:
        vm_fl = 1.0
    sc_f1 = ax_f1.scatter(
    ...
    ax_t0.scatter(
        pos[0][~is_sig], pos[1][~is_sig], marker=".", color="gray", s=15, alpha=0.5
    )
    ...
    if np.sum(is_sig) > 0:
        ...
        vm_tm = max(
            abs(np.nanpercentile(valid_res, 5)), abs(np.nanpercentile(valid_res, 95)), 5
        )
        sc_t1 = ax_t1.scatter(
            pos[0][is_sig],
            pos[1][is_sig],
            c=valid_res,
            cmap=PLOT_PARAMS["cmap_tm_res"],
            s=size,
            vmin=-vm_tm,
            vmax=vm_tm,
            edgecolors="none",
        ...
        ax_t1.scatter(
            pos[0][~is_sig], pos[1][~is_sig], marker="x", color="gray", s=15, alpha=0.5
        )
        fig.colorbar(sc_t1, ax=ax_t1, label="Obs$-$Pred [ns]")
    _setup_ax_fancy(
        ax_t1, "Timing Residuals", extent, show_xlabel=False, show_ylabel=False
    )
    ...
    if cf_stats and "timing_mean" in cf_stats:
        ...
        t_vlim = (
            max(abs(np.nanmin(t_map)), abs(np.nanmax(t_map)), 1.0)
            if t_map.size > 0
            else 1.0
        )
        im_t2 = ax_t2.imshow(
    ...
    _plot_kde_fancy(
        ...
        mc_truth=(mc_truth or {}).get("xmax_gpcm2"),
        mc_truth_fmt="%.1f g/cm$^2$",
    )
    ...
    _mc_energy = (mc_truth or {}).get("energy_ev")
    if mc_truth is not None:
        # No LORA curve on a simulation plot
        truth_e, truth_e_std = None, None
    else:
        truth_e = (
            att_truth["logE"]
            if att_truth
            else (
                np.log10(ref_params["energy"])
                if ref_params.get("energy", 0) > 0
                else 17.0
            )
        )
        truth_e_std = att_truth["logE_std"] if att_truth else 0.086
    _plot_kde_fancy(
        ax_en, log_e_samples, "IFT", truth_mean=truth_e, truth_std=truth_e_std,
        mc_truth=(np.log10(_mc_energy) if _mc_energy else None),
        mc_truth_fmt="%.3f",
    )
    ...
    _plot_2d_contour_fancy(
        ...
        y_label="Core Y [m]",
        title="Core Position",
        truth_point=((mc_truth or {}).get("core_x_m"),
                     (mc_truth or {}).get("core_y_m")) if mc_truth else None,
    )
    ...
    az_samples = (
        np.rad2deg(
            [
                float(np.asarray(signal_response.zen_and_az(s)[1]).item())
                for s in samples
            ]
        )
        % 360.0
    )
    zen_samples = np.rad2deg(
        [float(np.asarray(signal_response.zen_and_az(s)[0]).item()) for s in samples]
    )
    _plot_2d_contour_fancy(
        ax_dir,
        az_samples,
        zen_samples,
        old_point=(
            (np.rad2deg(direction_reference[1]) % 360.0,
             np.rad2deg(direction_reference[0]))
            if direction_reference is not None
            else (cmp_azimuth_deg, cmp_zenith_deg)
        ),
        reference_label=(
            direction_reference_label if direction_reference is not None else ref_label
        ),
        x_label="Azimuth [deg]",
        y_label="Zenith [deg]",
        title="Arrival Direction",
        truth_point=(
            (np.rad2deg(mc_truth["azimuth_rad"]) % 360.0,
             np.rad2deg(mc_truth["zenith_rad"]))
            if mc_truth and mc_truth.get("zenith_rad") is not None
            else None
        ),
    )

    # REJECTED stamp. Drawn over the whole figure
    ...
    if rejection_reason:
        ...
        stamp = fig.text(
            0.5, 0.5, "REJECTED",
            fontsize=240, fontweight="bold", color="grey", alpha=0.5,
            ha="center", va="center", rotation=30, zorder=1000,
        )

        def _fit_width(artist, fraction=0.9):
            fig.canvas.draw()
            bbox = artist.get_window_extent(fig.canvas.get_renderer())
            room = fraction * fig.bbox.width
            if bbox.width > room:
                artist.set_fontsize(artist.get_fontsize() * room / bbox.width)

        _fit_width(stamp)


        _angle = np.radians(30.0)
        _reason = textwrap.fill(rejection_reason, 70)
        _reason_fs = max(stamp.get_fontsize() / 7.0, 14.0)

        _stamp_h = (stamp.get_fontsize() / 72.0) / fig.get_figheight()
        _reason_h = (
            (_reason.count("\n") + 1) * 1.25 * _reason_fs / 72.0
        ) / fig.get_figheight()
        _offset = 0.5 * _stamp_h + 0.5 * _reason_h + 0.012
        reason_text = fig.text(
            0.5 + _offset * np.sin(_angle),
            0.5 - _offset * np.cos(_angle),
            _reason,
            fontsize=_reason_fs,
            fontweight="bold", color="grey", alpha=0.5,
            ha="center", va="center", rotation=30, zorder=1001,
            multialignment="center",
        )
        _fit_width(reason_text, fraction=0.8)
