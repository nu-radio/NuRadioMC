...
_PACKAGE_IFT_DATA_DIR = Path(__file__).resolve().parents[1] / "data" / "LOFAR" / "ift"
...
def hp_get_magnetic_field_vector(site: str = "lofar") -> jax.Array:
    ...
    magnetic_fields = {
        ...
        "summit": jnp.array([-0.037467, 0.075575, -0.539887]),
...
class CStrafoJAX:
    """JAX coordinate transformations for air-shower radio detection.
    ...
    def __init__(self, zenith, azimuth, magnetic_field_vector=None, site=None):
        ...
        self.transformation_matrix_onsky = jnp.stack(
            [e1_onsky, e2_onsky, e3_onsky], axis=-2
        )
        ...
        self.transformation_matrix_magnetic = jnp.array(
            [  # Mag -> Geo
                [c, -s, 0],
                [s, c, 0],
                [0, 0, 1],
        ...
        self.inverse_transformation_matrix_magnetic = (
            self.transformation_matrix_magnetic.T
        )
        angle = -azimuth
        c = jnp.cos(angle)
        s = jnp.sin(angle)
        ...
        # Azimuth CS -> Geo
        self.inverse_transformation_matrix_azimuth = (
            self.transformation_matrix_azimuth.T
        )

        # Transformation matrix from ground (geographic) CS to shower plane (early-late)
        # CS
        # z-axis is parallel to shower axis, shower axis projection on ground is in
        # yz-plane
        # Step 1: Rotate around Z by (-azimuth + pi/2) -> aligns Y axis with ground
        # projection
        ...
        self.inverse_transformation_matrix_early_late = (
            self.transformation_matrix_early_late.T
        )
    ...
    def _transform(self, positions, matrix):
        ...
        if is_single_vector:
            ...
            if (
                ...
                raise ValueError(
                    f"Expected (3,), (N,3) or (3,Nsamples>3); got {original_shape}"
                )
    ...
    def transform_from_vxB_vxvxB(self, positions, core=None):
        ...
        transformed_pos = self._transform(
            positions, self.inverse_transformation_matrix_vBvvB
        )
        ...
        if core is not None:
            ...
            if (
                transformed_pos.ndim == 2
                and transformed_pos.shape[0] == 3
                and core.shape == (3,)
            ):
                transformed_pos += core[
                    :, jnp.newaxis
                ]  # Add core to each time sample's vector
            else:
    ...
    def transform_from_vxB_vxvxB_2D(self, positions_2d, core=None):
        ...
        x_sp = positions_2d[:, 0]
        y_sp = positions_2d[:, 1]
        z_sp = self.get_height_in_showerplane(
            x_sp, y_sp
        )  # Calculate corresponding z_sp
        ...
        positions_3d_geo = self._transform(
            positions_3d_sp, self.inverse_transformation_matrix_vBvvB
        )
...
with open(get_ift_data_path("geo_rcut_b_splines.pickle"), "rb") as fin:
    ...
    jax_spline_data["rcut_geo"] = (
        jnp.array(_pad_knots(t_rcut, k_rcut)),
        jnp.array(c_rcut),
        k_rcut,
    )
...
# Uncalibrated linear-extrapolation margin [g/cm^2] at each spline-domain end.
# Exclude this margin when deriving Xmax prior bounds.
LDF_SPLINE_EXTRAP_MARGIN_GPCM2 = 75.0


def ldf_dxmax_valid_range(include_extrapolation_margin=False):
    """Common dxmax domain [g/cm^2] over which the LDF shape splines were fitted.

    Two distinct failure modes live outside it.  Beyond the knot domain
    ``evaluate_bspline`` clips its argument instead of extrapolating, so the
    footprint shape stops changing with dxmax altogether and the fit slides to
    the prior edge at no cost.  Inside the last ``LDF_SPLINE_EXTRAP_MARGIN_GPCM2``
    before that, the spline is a linear continuation of the fit. Both are excluded here.

    Pass ``include_extrapolation_margin=True`` for the raw knot domain.  Callers
    that put a prior on Xmax should intersect it with this window; see
    ``iftReconstructor._xmax_prior_bounds``.
    """
    lo, hi = -np.inf, np.inf
    for key in LDF_SHAPE_SPLINE_KEYS:
        t, _c, k = get_jax_spline_tck(key)
        t = np.asarray(t)
        lo = max(lo, float(t[k]))
        hi = min(hi, float(t[-(k + 1)]))
    if not include_extrapolation_margin:
        margin = LDF_SPLINE_EXTRAP_MARGIN_GPCM2
...
def get_lorentz_force_vector(
    zenith: ArrayLike, azimuth: ArrayLike, magnetic_field_vector: ArrayLike
) -> jnp.ndarray:
    ...
    magnetic_field_vector_normalized = jnp.where(
        norm > 1e-9, magnetic_field_vector / norm, magnetic_field_vector * 0.0
    )  # Zero vector if norm is zero
    return jnp.cross(showerAxis, magnetic_field_vector_normalized)


def get_sine_angle_to_lorentz_force(
    zenith: ArrayLike, azimuth: ArrayLike, magnetic_field_vector: ArrayLike = None
) -> ArrayLike:
    """Return the sine of the shower-axis/Lorentz-force angle with JAX."""
    if magnetic_field_vector is None:
...
def get_a(
    ...
    norm_factor = jnp.where(
        magnetic_field_strength > 1e-9, (magnetic_field_strength / 0.243) ** 0.9, 1.0
    )
    # Handle case where B=0, should norm_factor be infinity or 1? If B=0, Egeo=0 anyway.
    # Let's use 1.
...
def get_p(r: ArrayLike, rcut: ArrayLike, b_param: ArrayLike) -> ArrayLike:
    ...
    p_geo = 2.0 * jnp.nan_to_num(
        p_geo_base, nan=1.0, posinf=1.0, neginf=1.0
    )  # If pow fails, default to factor 1 -> p_geo=2

    # Calculate r^(-b_param) safely
    r_pow_neg_b = jnp.power(jnp.maximum(r, 1e-9), -b_param)  # Avoid r=0
    r_pow_neg_b = jnp.nan_to_num(
        r_pow_neg_b, nan=0.0, posinf=0.0, neginf=0.0
    )  # If pow fails, default to 0?

    # Use jnp.where for the conditional based on r vs rcut
    p = jnp.where(r <= rcut, 2.0, p_geo * r_pow_neg_b)
    return p
...
def LDF_vB_parts(
    ...
    exponent_term = jnp.nan_to_num(
        exponent_term, nan=jnp.inf, posinf=jnp.inf
    )  # Map NaNs/Infs from pow to Inf
...
def LDF_vB(
    ...
    """Geomagnetic LDF component base function."""
    x = jnp.asarray(x)
    y = jnp.asarray(y)
    sigma = jnp.maximum(
        jnp.asarray(sigma), 1e-12
    )  # Avoid sigma=0, use smaller epsilon for stability
    R_val = jnp.asarray(R_val)
    E = jnp.asarray(E)
    p = jnp.asarray(p)
    ...
    denom_inner = (
        jax.scipy.special.erf(arg_erf) * sqrtpi * sqrt2 * exp_term_norm2 * R_val
        + 2.0 * sigma
    ) * jnp.pi
    ...
    norm_factor2 = jnp.nan_to_num(
        norm_factor2, nan=0.0, posinf=0.0, neginf=0.0
...
def my_gamma2(
    ...
    """Charge Excess LDF component base function."""
    xx = jnp.asarray(xx)
    E = jnp.asarray(E)
    ...
    gamma_arg = 0.5 * k + 1.0
    gamma_val = jax.scipy.special.gamma(
        jnp.maximum(gamma_arg, 1e-6)
    )  # Avoid gamma(<=0)
    ...
    term_pow_neg_half_k = jnp.power(
        jnp.maximum(term_pow_neg_half_k_base, 1e-9), -0.5 * k
    )
    ...
    norm_denominator = jnp.maximum(
        jnp.abs(norm_denominator), 1e-12
    )  # Avoid division by zero
    ...
    exp_exponent_denom = jnp.maximum(
        jnp.abs(exp_exponent_denom), 1e-12
    )  # Avoid division by zero
    ...
    r_pow_k = jnp.where(
        r_abs < 1e-12, 0.0, r_pow_k
    )  # Ensure 0^k is handled (gives 0 if k>0)
...
def LDF_geo_dxmax(
    x: ArrayLike, y: ArrayLike, dxmax: ArrayLike, E: ArrayLike
) -> ArrayLike:
    """Geomagnetic LDF using dxmax parametrization."""
    x = jnp.asarray(x)
    y = jnp.asarray(y)
    dxmax = jnp.asarray(dxmax)
    E = jnp.asarray(E)
...
def LDF_ce_dxmax(
    ...
    """Charge Excess LDF using dxmax parametrization."""
    x = jnp.asarray(x)
    y = jnp.asarray(y)
    dxmax = jnp.asarray(dxmax)
    E = jnp.asarray(E)
...
def _LDF_core(
    ...
    core_in, magnetic_field_vector_in = (
        jnp.asarray(core),
        jnp.asarray(magnetic_field_vector),
    )
    ...
    sinalpha = get_sine_angle_to_lorentz_force(
        zenith_in, azimuth_in, magnetic_field_vector_in
    )
