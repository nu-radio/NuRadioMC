...
# also a severe local outlier. Prevents far out stations from being removed
# when their timing is off (old data)
...
# Trace fraction skipped at each end, where the band-pass and taper suppress
# power. Never less than the taper itself, so lowering the literal cannot
# silently start measuring noise inside the roll-off.
MAX_SIGNAL_EDGE_FRACTION = max(0.125, BANDPASS_HALF_HANN_PERCENT)
...
def get_electric_field_energy_fluence(
    ...
    """Integrate electric-field energy fluence !!without!! any noise subtraction."""
    electric_field_trace = np.asarray(electric_field_trace)
    times = np.asarray(times)
    if electric_field_trace.size == 0 or times.size < 2:
        return np.zeros(
            electric_field_trace.shape[0] if electric_field_trace.ndim > 1 else 1
        )
    ...
    if signal_window_mask is None:
        ...
        f_signal = np.sum(
            np.abs(electric_field_trace[:, signal_window_mask]) ** 2, axis=1
        )
...
def station_beamform_pulse_finder(
    ...
    shower_zen,
    shower_az,
    scan_range_deg=DEFAULT_SCAN_RANGE_DEG,
    ...
    az_grid = shower_az + np.deg2rad(
        np.arange(-scan_range_deg, scan_range_deg + 0.1, step_deg)
    )
    zen_grid = shower_zen + np.deg2rad(
        np.arange(-scan_range_deg, scan_range_deg + 0.1, step_deg)
    )
    ...
    # Precompute all per-antenna delays for every grid direction in one vectorised
    # batch.
        ...
        for i_az, azimuth in enumerate(az_grid):
            ...
            for cluster_indices in clusters:
                ...
                noise_section = (
                    ...
                    if peak_idx > n_samples // 2
                    else beamformed_sum[-n_samples // 4 :]
                )
                noise_std = np.std(noise_section)
                current_snr = (
                    0.0
                    if noise_std < 1e-12
                    else (beamformed_sum[peak_idx] - np.mean(noise_section)) / noise_std
                )
...
def power_peak_finder(voltage_traces, voltage_times):
    ...
    noise_est = (
        np.std(power_trace[: len(power_trace) // 5])
        if len(power_trace) > 5
        else np.std(power_trace)
    )
    snr = smoothed_power[peak_idx] / noise_est if noise_est > 1e-12 else float("inf")
    return voltage_times[peak_idx], snr


def _neighbor_timing_outlier_test(
    positions, times, point_index, n_neighbors=OUTLIER_NEIGHBORS_K
):
    n_points = times.size
    if n_points <= 1:
...
def _high_snr_timing_point_can_be_demoted(
    global_index, positions, times, candidate_mask, station_ids, timing_snrs
):
...
def detect_timing_outliers(positions, times, station_ids, timing_snrs=None):
    ...
    for station_id in np.unique(station_ids):
        ...
        if k_for_station < MIN_ABSOLUTE_NEIGHBORS:
            continue
        time_std_ns = np.std(
            (station_times - np.mean(station_times)) * units.s / units.ns
        )
        ...
        for index in range(len(station_indices)):
            ...
            if is_outlier and timing_snrs is not None:
                ...
                if (
                    np.isfinite(point_snr)
                    and point_snr >= HIGH_SNR_TIMING_PROTECTION_SNR
                ):
                    _, _, is_outlier = _neighbor_timing_outlier_test(
                        station_positions, station_times, index
                    )
            if is_outlier:
    ...
    logger.info(
        "Identified %d timing outliers for demotion.", times.size - np.sum(keep_mask)
    )
    return keep_mask


def iterative_timing_pruning(
    shower_direction, positions, times, initial_mask, station_ids=None, timing_snrs=None
):
    """Iteratively prune timing residual outliers with a local polynomial time model.

    High-SNR points are skipped unless severe outliers in the neighbor test.
    """
    ...
    for _ in range(max_removals):
        ...
        if len(active_indices) < MIN_TIMING_POINTS:
            break
        result = _fit_timing_polynomial(
            positions[:, active_indices], times[active_indices]
        )
        residuals = result["residuals"]
        std_residual = np.std(residuals)
        if std_residual < TIMING_UNCERT_THRESHOLD:
            logger.info(
                "Timing pruning converged. Std: %.2f ns. Points remaining: %d",
                std_residual * units.s / units.ns,
                len(active_indices),
            )
            return (
                current_mask,
        ...
        if not removed_point:
            logger.warning(
                "Timing pruning stopped: remaining candidates have protected high SNR."
            )
            break

    if protected_skips > 0:
        logger.info(
            "High-SNR timing protection skipped %d global pruning removals.",
            protected_skips,
        )
...
def _fit_timing_polynomial(active_pos, active_times):
    ...
    matrix = np.column_stack(
        [np.ones(len(active_times)), x_norm, y_norm, x_norm**2 + y_norm**2]
    )
...
def get_local_timing_uncertainties(
    ...
    station_ids,
    shower_direction,
    min_uncertainty_s=1.5 * units.ns / units.s,
    cluster_size=20,
):
    ...
    for index in range(n_antennas):
        ...
        if np.size(neighbor_indices) < 8:
            ...
            continue
        result = _fit_timing_polynomial(
            positions[:, neighbor_indices], times[neighbor_indices]
        )
...
def extract_fluence_and_timing_data(
    ...
    Returns posx, posy, fluences, max_times, is_signal, noise_fluences,
    snr_values and best_direction.
    """
    ...
    try:
        ...
        for ch in station.iter_channels():
            ...
            try:
                ...
                bf_pos_list.append(
                    (
                        detector.get_relative_position(station_id, ch_id)
                        + station_abs_pos
                    )[:2]
                )
            except Exception:
    ...
    for ef in station.get_electric_fields():
        try:
            ...
            ch_id_even = ch_ids[0]
            position = (
                detector.get_relative_position(station_id, ch_id_even) + station_abs_pos
            )
            try:
                ch_even_obj = station.get_channel(ch_id_even)
                ch_odd_obj = station.get_channel(ch_id_even + 1)
                full_voltage_traces = np.array(
                    [ch_even_obj.get_trace(), ch_odd_obj.get_trace()]
                )
            ...
            if global_ref_time is not None:
                ant_roi_center = global_ref_time + (
                    ant_delay_nuradio - ref_delay_nuradio
                )
            else:
            ...
            time_mask = (full_voltage_times >= ant_roi_start) & (
                full_voltage_times <= ant_roi_end
            )
            ...
            antenna_data.append(
                {
                    ...
                    "current_max_snr": current_max_snr,
                    "efield_trace": efield_trace,
                    "efield_times": efield_times,
                }
            )
        except Exception as exc:
            logger.warning(
                "Could not process E-field for station %s: %s", station_id, exc
            )
    ...
    for ant in antenna_data:
        ...
        if max_time_nuradio is not None:
            ...
            fl = (
                np.sum(
                    get_electric_field_energy_fluence(
                        efield_trace[:, sig_mask], efield_times[sig_mask]
                    )
                )
                if np.any(sig_mask)
                else 0.0
            ...
            noise_fluences.append(
                _noise_fluence_windows(
...
# Noise windows are tiled outside the bandpass roll-off and away from the pulse.

_BANDPASS_EDGE_SAMPLES_START = 20000
_BANDPASS_EDGE_SAMPLES_END = 45536

_SHORT_TRACE_EDGE_FRACTION = 0.05
_SHORT_TRACE_MIN_EDGE_SAMPLES = 8

# Half-width of the excluded region around the pulse. A 30-80 MHz band-limited
# pulse rings well past its envelope, so a window butted up against the peak
# still catches signal and biases the floor high.
_PULSE_GUARD_WINDOWS = 1.0
_PULSE_GUARD_MIN_NS = 300.0

_NOISE_WINDOW_STRIDE_FRACTIONS = (0.5, 0.25, 0.125)
_MIN_NOISE_WINDOWS = 6
_PULSE_GUARD_FLOOR_WINDOWS = 0.5


def _noise_fluence_windows(
    efield_trace, efield_times, max_time_nuradio, fluence_window_ns
):
    """Noise fluences for ONE antenna, in windows as long as the signal window.

    Returns a plain list. It may be shorter for one antenna than another --
    traces differ by a sample or two -- and that is now fine: the caller keeps
    the per-antenna grouping explicitly. It used to infer the grouping from a
    divisibility check, which failed whenever the counts were unequal and then
    silently threw the whole per-antenna floor away (24% of showers).
    """
    ...
    if time_step_nuradio <= 0:
        return []
    fluence_window_nuradio = fluence_window_ns * units.ns
    window_length_samples = int(round(fluence_window_nuradio / time_step_nuradio))
    if window_length_samples <= 0:
        return []

    n_total = len(efield_times)
    if n_total > _BANDPASS_EDGE_SAMPLES_START + window_length_samples:
        taper_samples = int(np.ceil(BANDPASS_HALF_HANN_PERCENT * n_total))
        edge_start = max(_BANDPASS_EDGE_SAMPLES_START, taper_samples)
        edge_end = min(_BANDPASS_EDGE_SAMPLES_END, n_total - taper_samples)
    else:
        edge_start = max(
            _SHORT_TRACE_MIN_EDGE_SAMPLES,
            int(round(_SHORT_TRACE_EDGE_FRACTION * n_total)),
        )
        edge_end = n_total - edge_start

    def tile(guard, stride):
        out = []
        for start_index in range(
            edge_start, edge_end - window_length_samples + 1, stride
        ):
            end_index = start_index + window_length_samples
            if (
                efield_times[end_index - 1] >= max_time_nuradio - guard
                and efield_times[start_index] <= max_time_nuradio + guard
            ):
                continue  # overlaps the guarded region around the pulse
            out.append(
                np.sum(
                    get_electric_field_energy_fluence(
                        efield_trace[:, start_index:end_index],
                        efield_times[start_index:end_index],
                    )
                )
            )
        return out
    ...
    guard = max(
        _PULSE_GUARD_WINDOWS * fluence_window_nuradio,
        _PULSE_GUARD_MIN_NS * units.ns,
    )
    guard_floor = _PULSE_GUARD_FLOOR_WINDOWS * fluence_window_nuradio
    best = []
    while True:
        for frac in _NOISE_WINDOW_STRIDE_FRACTIONS:
            windows = tile(guard, max(1, int(round(frac * window_length_samples))))
            if len(windows) >= _MIN_NOISE_WINDOWS:
                return windows
            if len(windows) > len(best):
                best = windows
        if guard <= guard_floor:
            return best
        guard = max(guard_floor, 0.75 * guard)


def blind_max_signal_finder(
    voltage_traces, voltage_times, edge_fraction=MAX_SIGNAL_EDGE_FRACTION
):
...
def plane_wave_consensus_mask(
    posx, posy, times, shower_direction, window_ns=MAX_SIGNAL_CONSENSUS_WINDOW_NS
):
...
def extract_max_signal_timing_data(
    ...
    shower_direction,
    fluence_window_ns=None,
    snr_threshold=MAX_SIGNAL_SNR_THRESHOLD,
):
    ...
    for efield in station.get_electric_fields():
        ...
        try:
            ...
            voltage_traces = np.array(
                [channel_even.get_trace(), channel_odd.get_trace()]
            )
        ...
        position = (
            detector.get_relative_position(station_id, channel_id_even)
            + station_abs_pos
        )
        candidates.append(
            {
    ...
    logger.info(
        "Station %s: blind search kept %d antennas (of %d above SNR %.1f).",
        station_id,
        len(fluences),
        len(candidates),
        snr_threshold,
...
def augment_neighbor_fluences(
    ...
    fluence_window_ns,
    known_positions_set,
    max_neighbor_dist_m=100.0,
):
    """Add fluence-only antennas near existing signal antennas.

    Add unextracted antennas within max_neighbor_dist_m of a signal antenna.
    Integrate at the plane-wave arrival time. Mutates known_positions_set.
    """
    new_data = {
        "posx": [],
        "posy": [],
    ...
    for station_id in station_ids_to_check:
        ...
        for ef in station.get_electric_fields():
            try:
                ...
                ch_even = ch_ids[0]
                abs_pos = (
                    detector.get_relative_position(station_id, ch_even)
                    + station_abs_pos
                )
                pos_xy = abs_pos[:2]
                ...
                geom_delay_s = -(1.0 / scipy.constants.c) * (
                    kx * pos_xy[0] + ky * pos_xy[1]
                )
                ...
                fl = (
                    np.sum(
                        get_electric_field_energy_fluence(
                            efield_trace[:, mask], efield_times[mask]
                        )
                    )
                    if np.sum(mask) >= 2
                    else 0.0
...
def count_efield_triggered_stations(
    fluences,
    station_ids,
    noise_mean,
    reconstruction_window_ns,
...
def filter_samples_by_trigger(
    model,
    samples_list,
    station_ids,
    noise_mean,
    reconstruction_window_ns,
    min_stations=TRIGGER_MIN_STATIONS,
    snr_buffer_frac=TRIGGER_SNR_BUFFER_FRAC,
    ...
    logger.info(
        "Trigger: win=%g ns, noise(55ns)=%.2f, thresh=%.2f, buffer=%.0f%%, "
        "min_stations=%d",
        reconstruction_window_ns,
        noise_55ns,
        threshold_fluence,
    ...
    for i in range(len(samples_list)):
        ...
        for sid in unique_stations:
            ...
            if len(st_fluences) == 0:
                continue
            if (
                np.sum(st_fluences > threshold_fluence) / len(st_fluences)
            ) >= TRIGGER_ANTENNA_FRACTION:
...
def uniform_prior_latent(value, prior):
    ...
    """
    frac = (float(value) - float(prior["a_min"])) / (
        float(prior["a_max"]) - float(prior["a_min"])
    )
...
def _observable_arrays(
    ...
    return (
        ...
        _noise_blocks(noise_fluences),
        np.asarray(snr_values),
    )


def _noise_blocks(noise_fluences):
    """Stack per-antenna noise-window lists into one (n_antenna, n_window) array.

    Antennas with fewer windows are padded with NaN, so the block keeps its
    row-per-antenna meaning without requiring every antenna to have produced
    the same number of windows. Consumers must use the nan-aware reductions.
    """
    rows = [np.asarray(w, dtype=float).reshape(-1) for w in noise_fluences]
