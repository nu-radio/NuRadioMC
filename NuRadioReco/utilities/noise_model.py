...
class NoiseModel:
    ...
    def calculate_minus_two_delta_llh_stations(self, station, sim_station_list, time_grid, use_channels = None, frequency_domain=False, plot=True, return_traces=False):
        ...
        ----------
            ...
            sim_station_list : list
                List of NuRadioReco.framework.base_station.Station objects containing channels for the signal and e.g. different ray-tracing solutions. Length of
                this list should be the same as the length of use_channels.
            use_channels : list
        ...
        t_0_sim = list(sim_station_list[0].iter_channels())[0].get_trace_start_time() # There is probably a better way to do this
        ...
        for i_time, time_offset in enumerate(np.atleast_1d(time_grid)):
            ...
            for i_ant, sim_station in enumerate(sim_station_list):
                if not isinstance(sim_station, NuRadioReco.framework.sim_station.SimStation):
                    continue
                # Make empty channel:
                signal_readout_channel = NuRadioReco.framework.channel.Channel(1)
                signal_readout_channel.set_trace(np.zeros(self.n_samples), self.sampling_rate)
