...
def load_system_response(path=os.path.dirname(os.path.realpath(__file__)), **kwargs):
    ...
    ARA_config_id  = kwargs.get("ARA_config_id")
    if ARA_config_id == 'C0':
       data = np.loadtxt(os.path.join(path, "HardwareResponses/ARA_default_configuration.txt"),
                      skiprows=3, delimiter=' ') # Proceed with default gain (ARAsim default gain)
    else:
       data = np.loadtxt(os.path.join(path, "HardwareResponses/ARA_"+str(station_id)+"_configuration_"+str(ARA_config_id)+".txt"),
...
def get_system_response(frequencies, station, evt_time):
    ...
    if station_id in collect_station_id:
       ...
       ARA_config_id = ARA_stations_configuration.get_ARA_hardware_configuration(station_id, evt_time)
       system_response['default'] = load_system_response(station_id= station_id,  ARA_config_id = ARA_config_id)
