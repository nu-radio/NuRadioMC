...
def create_single_plot(S_names, plot_info_cont, data, xlabel, ylabel, xunit, yunit):
    ...
    if data != []:
        data_x = data[0]
        data_y = data[1]
        fig.append_trace(go.Scatter(x=np.asarray(data_x) / str_to_unit[xunit], y=np.asarray(data_y) / str_to_unit[yunit], opacity=0.7, marker={'color': "blue", 'line': {'color': "blue"}}, name='magnitude'), 1,
                         1)
        fig['layout']['xaxis1'].update(title=f'{xlabel} [{xunit}]')
        fig['layout']['yaxis1'].update(title=f'{ylabel} [{yunit}]')
        plot_info_cont.info(
            f'you entered {len(data_x)} {xlabel} from {min(data_x) / str_to_unit[xunit]:.4g}{xunit} to {max(data_x) / str_to_unit[xunit]:.4g}{xunit} and {S_names} {ylabel} {len(data_y)} values within the range of {min(data_y) / str_to_unit[yunit]:.4g}{yunit} to {max(data_y) / str_to_unit[yunit]:.4g}{yunit}')
...
def create_double_plot(S_names, plot_info_cont, data1, data2, labels1, labels2, units1, units2):
    ...
    if data1 != []:
        ...
        data1_y = data1[1]
        fig.append_trace(
            go.Scatter(x=np.asarray(data1_x) / str_to_unit[units1[0]], y=np.asarray(data1_y) / str_to_unit[units1[1]], opacity=0.7, marker={'color': "blue", 'line': {'color': "blue"}}, name='magnitude'), 1, 1)
        fig['layout']['xaxis1'].update(title=f'{labels1[0]} [{units1[0]}]')
        fig['layout']['yaxis1'].update(title=f'{labels1[1]} [{units1[1]}]')
        plot_info_cont.info(
            f'you entered {len(data1_x)} {labels1[0]} from {min(data1_x) / str_to_unit[units1[0]]:.4g}{units1[0]} to {max(data1_x) / str_to_unit[units1[0]]:.4g}{units1[0]} and {S_names} {labels1[1]} {len(data1[1])} values within the range of {min(data1_y) / str_to_unit[units1[1]]:.4g}{units1[1]} to {max(data1_y) / str_to_unit[units1[1]]:.4g}{units1[1]}')
    if data2 != []:
        data2_x = data2[0]
        data2_y = data2[1]
        fig.append_trace(go.Scatter(x=np.asarray(data2_x) / str_to_unit[units2[0]], y=np.asarray(data2_y) / str_to_unit[units2[1]], opacity=0.7, marker={'color': "red", 'line': {'color': "red"}}, name='phase'),
                         1, 2)
        fig['layout']['xaxis2'].update(title=f'{labels2[0]} [{units2[0]}]')
        fig['layout']['yaxis2'].update(title=f'{labels2[1]} [{units2[1]}]')
        plot_info_cont.info(
            f'you entered {len(data2_x)} {labels2[0]} from {min(data2_x) / str_to_unit[units2[0]]:.4g}{units2[0]} to {max(data2_x) / str_to_unit[units2[0]]:.4g}{units2[0]} and {S_names} {labels2[1]} {len(data2_y)} values within the range of {min(data2_y) / str_to_unit[units2[1]]:.4g}{units2[1]} to {max(data2_y) / str_to_unit[units2[1]]:.4g}{units2[1]}')
...
def create_ten_plots(S_names, plot_info_cont, data, xlabels, ylabels, input_units, groupdelay_correction):
    if data != []:
        ...
        fig = subplots.make_subplots(rows=6, cols=2, specs=[[{"rowspan": 2}, {"rowspan": 2}], [None, None], [{}, {}], [{}, {}], [{}, {}], [{}, {}]], subplot_titles=(subtitles))
        fig.append_trace(go.Scatter(x=data[0] / str_to_unit[frequency_unit], y=calc_corr_group_delay_0 / str_to_unit[group_delay_unit], opacity=0.7, marker={'color': "red", 'line': {'color': "red"}}), 1, 1)
        fig.append_trace(go.Scatter(x=data[0] / str_to_unit[frequency_unit], y=calc_corr_group_delay / str_to_unit[group_delay_unit], opacity=0.7, marker={'color': "green", 'line': {'color': "green"}}), 1, 2)
        for i in range(4):
            fig.append_trace(go.Scatter(x=data[0] / str_to_unit[frequency_unit], y=data[i * 2 + 1] / str_to_unit[input_units[1]], opacity=0.7, marker={'color': "blue", 'line': {'color': "blue"}}), i + 3, 1)
            fig.append_trace(go.Scatter(x=data[0] / str_to_unit[frequency_unit], y=data[i * 2 + 2] / str_to_unit[input_units[2]], opacity=0.7, marker={'color': "red", 'line': {'color': "red"}}), i + 3, 2)
        ...
        for isn, sname in enumerate(S_names):
            plot_info_cont.info(
                f'{sname}: {ylabels[1]} {len(data[isn * 2 + 1])} values within the range of {min(data[isn * 2 + 1]) / str_to_unit[input_units[1]]:.4g}{input_units[1]} to {max(data[isn * 2 + 1]) / str_to_unit[input_units[1]]:.4g}{input_units[1]}'
