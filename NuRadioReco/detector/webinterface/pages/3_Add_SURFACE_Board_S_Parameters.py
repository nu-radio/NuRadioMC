...
def build_main_page(main_cont):
    main_cont.title('Add S parameter measurement of SURFACE board')
    main_cont.markdown(page_name)
    cont_warning_top = main_cont.container()

    # select the cable
    surface_name, surface_dropdown, channel_id, temp = select_surface(page_name, main_cont, cont_warning_top)
    ...
    disable_button = validate_global_surface(page_name, cont_warning_bottom, surface_dropdown, surface_name, channel_id, working, sdata_validated, uploaded_data)
    ...
    if upload_button:
        ...
        insert_surface_to_db(page_name, s_name, surface_name, S_data, input_units, working, primary, protocol, channel_id, temp, measure_time, group_delay_arr)
        main_cont.empty()
