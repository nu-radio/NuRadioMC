...
def build_main_page(main_cont):
    main_cont.title('Add S11 measurement of HPol Antenna')
    main_cont.markdown(page_name)
    cont_warning_top = main_cont.container()

    # select the Antenna name
    HPol_name, HPol_dropdown, HPol_text = select_antenna_name(page_name, main_cont, cont_warning_top)
    ...
    protocol = main_cont.selectbox('Specify the measurement protocol:', ['PennState2021', 'PennState2022'])
    ...
    if upload_button:
        insert_to_db(page_name, s_name, HPol_name, S_data, working, primary, protocol, input_units)
