...
def build_main_page(main_cont):
    main_cont.title('Add S21 measurements for a SURFACE CABLE')
    main_cont.markdown(page_name)
    cont_warning_top = main_cont.container()

    # select the cable
    # surfcable_type, surfcable_station, surfcable_channel, surfcable_name= select_cable(page_name, main_cont, cont_warning_top)
    surfcable_name = select_cable(page_name, main_cont, cont_warning_top)
    ...
    # disable_button = validate_global_cable(cont_warning_bottom, surfcable_type, surfcable_station, surfcable_channel, working, smagnitude_validated, sphase_validated, uploaded_magnitude, uploaded_phase)
    disable_button = validate_global_cable(cont_warning_bottom, surfcable_name, working, smagnitude_validated, sphase_validated, uploaded_magnitude, uploaded_phase)
    ...
    if upload_button:
        insert_cable_to_db(page_name, s_name, surfcable_name, S_magnitude, S_phase, input_units, working, primary, protocol)
        main_cont.empty()
