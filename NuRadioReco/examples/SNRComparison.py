...
counter = -1
for E in 10 ** np.linspace(16, 18, 10):
    for theta in np.linspace(cherenkov_angle - 5 * units.deg, cherenkov_angle + 5 * units.deg, 10):
        ...
        SS_LPDA[counter] = np.sum(trace_LPDA ** 2)
        SS_bicone[counter] = np.sum(trace_bicone ** 2)
...
ax.set_ylabel(r"$\sum A_i^2 / V_\mathrm{RMS}^2$")
ax.set_xlabel(r"$V_\mathrm{p2p}/2/V_\mathrm{RMS}$")
ax.semilogx(True)
ax.semilogy(True)
ax2.set_ylabel(r"$\sqrt{\sum A_i^2 / V_\mathrm{RMS}^2}$")
ax2.set_xlabel(r"$V_\mathrm{p2p}/2/V_\mathrm{RMS}$")
ax2.scatter((SS_LPDA / Vrms ** 2) ** 0.5, Vp2p_LPDA / Vrms / 2, label='LPDA')
ax2.scatter((SS_bicone / Vrms ** 2) ** 0.5, Vp2p_bicone / Vrms / 2, label='bicone')
ax.legend()
ax2.set_xlim(0, 300)
ax2.set_ylim(0, 150)
ax2.legend()
