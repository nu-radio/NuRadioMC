...
Command line input:
    python FullReconstruction.py station_id input_files detector_file

station_id: int
            station id to be used, default 32
input_files: str
            CoREAS simulation file, default example data data,data,data
detector_file: str
...
try:
    ...
    input_files = sys.argv[2].split(',')  # file with coreas simulations
...
if(station_id == 32):
    ...
    triggered_channels = [1, 4, 7]
    used_channels_efield = [1, 4, 7]
    used_channels_fit = [1, 4, 7]
    channel_pairs = ((1, 4), (1, 7), (4, 7))
...
for iE, evt in enumerate(readCoREAS.run(detector=det)):
    ...
    if simulationSelector.run(evt, station.get_sim_station(), det):
        ...
        triggerSimulator.run(evt, station, det, passband=[80 * units.MHz, 280 * units.MHz], order=10, number_coincidences=2, threshold=0.08 * units.millivolt, coinc_window=220 * units.ns)

        if station.get_trigger('envelope_trigger').has_triggered():
