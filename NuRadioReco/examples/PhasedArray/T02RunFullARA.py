"""
This file runs ARA with a coincidence trigger on A1-4 and a phased array trigger on A5
simulation. The phased array configuration in this file is taken from the deployed 
ARA phased array: 1.5 GS/s, 8 antennas at a depth of ~175 m with a trigger threshold of
2 sigma, 15 phasing directions with primary and secondary beams.
...
class mySimulation(simulation.simulation):
    ...
    def _detector_simulation_part2(self):
        ...
        triggerSimulator.run(self._evt, self._station, self._det,
                             threshold=trigger_threshold * self._Vrms,  # see phased trigger module for explanation
                             triggered_channels=[16, 18, 20, 21, 22, 23, 24],  # run trigger the operational channels in the phased array
                             secondary_channels=None,  # secondary channels
                             trigger_name='nuPhase_primary_phasing',  # the name of the trigger
                             coupled=True,
                             cut_times=cut_times)
...
parser.add_argument('--detectordescription', type=str,
                    help='path to file containing the detector description', default='/Users/swissel/work/NuRadioReco/NuRadioReco/detector/ARA/ara_detector_db.json')
