    ...
    for thetamin, thetamax in zip(thetamins, thetamaxs):
        ...
        for flavour in flavours:
            ...
            for ifolder, Emin, Emax in zip(range(len(Emins)), Emins, Emaxs):
                folderlist.append(
                    "{:.2f}_{}_{}_{:.2e}_{:.2e}".format(thetamin / units.deg, flavour, str(ifolder).zfill(2), Emin,
                                                        Emax))
            ...
            for folder, Emin, Emax in zip(folderlist, Emins, Emaxs):
                ...
                generate_eventlist_cylinder(
                    ...
                    Emin,
                    Emax,
                    fiducial_zmin=volume['fiducial_zmin'],
                    fiducial_zmax=volume['fiducial_zmax'],
                    fiducial_rmin=volume['fiducial_rmin'],
                    fiducial_rmax=volume['fiducial_rmax'],
                    thetamin=thetamin,
                    thetamax=thetamax,
                    phimin=phimin,
                    phimax=phimax,
