...
for threshold_factor in threshold_factors:
    ...
    for Ntry in range(Ntries):
        ...
        for iant in range(len(primary_channels)):
            noise_array.append(
                channelGenericNoiseAdder.bandlimited_noise(min_freq, max_freq, n_samples, sampling_rate, amplitude,
                                                           type='rayleigh'))
    ...
    print('Threshold factor: {:.2f}, Fraction of noise triggers: {:.8f}%, Noise trigger rate: {:.2f} Hz'.format(
        threshold_factor, prob_per_window * 100., trigger_frequency / units.Hz))
