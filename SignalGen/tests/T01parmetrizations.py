...
spec1 = AskaryanModule.get_frequency_spectrum(E, theta, ff, 1, n_index, R)
spec2 = param.get_frequency_spectrum(E, theta, ff, 1, n_index, R, model='Alvarez2012')
spec3 = param.get_frequency_spectrum(E, theta, ff, 1, n_index, R, model='Alvarez2000')
spec4 = param.get_frequency_spectrum(E, theta, ff, 1, n_index, R, model='ZHS')

fig, ax = plt.subplots(1, 1)
ax.plot(ff / units.MHz, np.abs(spec1[0]) / units.V * units.m, 'o', label='Askaryan module eR (no LPM)')
ax.plot(ff / units.MHz, np.abs(spec1[1]) / units.V * units.m, 'o', label='Askaryan module eTheta (no LPM)')
...
ax.plot(ff / units.MHz, np.abs(spec4) / units.V * units.m, '>', label='ZHS')
ax.semilogx(True)
ax.semilogy(True)
ax.set_xlim(10, 2e3)
...
spectrum = AskaryanModule.get_time_trace(E, theta, ff, 0, n=1.78, R=R)
ax.plot(tt / units.ns, np.abs(spectrum[1]), label="dt = {:.2f}ns, n={}".format(dt / units.ns, n_trace))
