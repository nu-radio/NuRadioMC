...
Double_t h = 1 * utl::mm; /**< RK4 step size is set to 1mm */

const Double_t tmax = 10 * utl::km; /**< Total "time" Rk4 will run for. In this case it is total meters before the RK4 loop stops if nothing else (i.e hitting the target or going out of scan region) stops it at first. */
 ...
 */
Double_t *getStartAngles(Double_t x0, Double_t z0, Double_t x1, Double_t z1,
		Int_t solnum);
 ...
 */
Double_t *getAngleCorrections(Int_t num, Int_t antco, Int_t D56co,
		Double_t test0, Double_t test1, Int_t psns, Int_t in, Int_t solnum);

/*! \brief This is the main function where all the calculations are done using the RK4 method. 
 *This function takes in the values of:
 *
 * @param c1 x position of transmitter
 * @param c2 y position of transmitter
 * @param c3 x position of transmitter
 * @param r1 y position of reciever
 ...
 *
 *Find Refracted Solutions, wsol==2
 *
 *Find Direct & Reflected Solutions, wsol==3
 *
...
Double_t* RayTraceRK4(Double_t c1, Double_t c2, Double_t c3, Double_t r1,
		Double_t r2, Double_t r3, Int_t wsol, Double_t *frequency,
		Int_t tausize) {
	...
	//Set the limits for the for-loop to what solutions you are looking for
	Int_t isol_strt = 0;
	Int_t isol_stop = 1;
	...
	////Make a File to store the TTree which will have all the variables. This feature has been disabled right now but can be used if someone wants.
	...
	//Set branches for the TTree
	TTree *chTreeR = new TTree("chTreeR", "Raytrace Sol. Info. tree");
	...
	//Set the RK4 varaibles and total steps for the while-loop (N_tot)
	...
	//The for loop where all the calculations are done for all the solutions
	for (Int_t isol = isol_strt; isol < isol_stop; isol++) {
		...
		if (isol == 1) {
			cout
					<< "~~~~~~~~~~~~~~~~~Finding REFLECTED solutions~~~~~~~~~~~~~~~"
					<< endl;
		}
		if (isol == 2) {
			cout
					<< "~~~~~~~~~~~~~~~~~Finding REFRACTED solutions~~~~~~~~~~~~~~~"
					<< endl;
		}
		...
		while (dhit == true) {
			...
			while (wloop < N_tot) {
				...
				K2_theta = h
						* ftheta(th0 + 0.5 * K1_theta, z0 + 0.5 * K1_theta);
				K3_theta = h
						* ftheta(th0 + 0.5 * K2_theta, z0 + 0.5 * K2_theta);
				K4_theta = h * ftheta(th0 + K3_theta, z0 + K3_theta);

				for (Int_t itau = 0; itau < tausize; itau++) {
					K1_tau[itau] = h * ftau(tau0[itau], z0, frequency[itau]);
					K2_tau[itau] = h
							* ftau(tau0[itau] + 0.5 * K1_tau[itau],
									z0 + 0.5 * K1_tau[itau], frequency[itau]);
					K3_tau[itau] = h
							* ftau(tau0[itau] + 0.5 * K2_tau[itau],
									z0 + 0.5 * K2_tau[itau], frequency[itau]);
					K4_tau[itau] = h
							* ftau(tau0[itau] + K3_tau[itau], z0 + K3_tau[itau],
									frequency[itau]);
				}
				...
				for (Int_t itau = 0; itau < tausize; itau++) {
					tau0[itau] = tau0[itau]
							- Coef
									* (K1_tau[itau]
											+ 2 * (K2_tau[itau] + K3_tau[itau])
											+ K4_tau[itau]);
				}

				time0 = time0
						+ Coef * (K1_time + 2 * (K2_time + K3_time) + K4_time);
				...
				//Store all the required variables when the target is hit
				if (fabs(z0 - z1) < 0.5 && fabs(dist - x0) < 0.5) {
					if (fabs(dist - x0) < 0.1 && fabs(z0 - z1) < 0.1) {
						if (sqrt(
								(z0 - z1) * (z0 - z1)
										+ (dist - x0) * (dist - x0)) < prdist) {
							prdist = sqrt(
									(z0 - z1) * (z0 - z1)
											+ (dist - x0) * (dist - x0));
							minx = x0;
				...
				if ((z0 - (lowerz)) < -2) {
					wloop = N_tot + 1;
					brloop = 1;
					cout << "Ray going straight down! Target has been missed!"
							<< endl;
					depthpass[i] = true;
			...
			//After each iteration select new angles and decrease the width of the cone
			if (chkn == 0 && i == 1 && brloop == 0) {
				...
				if (dz[1] > dz[0]) {
					...
					cout << "dz1>dz0" << endl;
					newang = testfir[0]
							+ (fabs(dz[0]) / zdiff) * (testfir[1] - testfir[0]);
				}
				if (dz[0] > dz[1]) {
					zdiff = dz[0] - dz[1];
					cout << "dz0>dz1" << endl;
					newang = testfir[1]
							+ (fabs(dz[1]) / zdiff) * (testfir[0] - testfir[1]);
				}
			...
			if ((surfhit == true) && (isol == 0 || isol == 2)) {
				Double_t *infir2 = getAngleCorrections(ndhit, antcor[0],
						D56cor[0], testfir[0], testfir[1], +1, i, isol);
				if (surfhit == true) {
...
Double_t *getDirAng(Double_t x0, Double_t z0, Double_t x1, Double_t z1) {
	...
	TF1 *fn =
			new TF1("fn",
					"log((((sqrt([0]*[0]-sin(x)*sin(x)*[1]*[1])+sqrt([2]*[2]-sin(x)*sin(x)*[1]*[1]))/([0]-[2]))+([2]/sqrt([2]*[2]-sin(x)*sin(x)*[1]*[1])))/(((sqrt([1]*[1]-sin(x)*sin(x)*[1]*[1])+sqrt([2]*[2]-sin(x)*sin(x)*[1]*[1]))/([1]-[2]))+([2]/sqrt([2]*[2]-sin(x)*sin(x)*[1]*[1]))))-((sqrt([2]*[2]-sin(x)*sin(x)*[1]*[1])*[3])/(sin(x)*[1]))",
					0, pi);
	fn->FixParameter(0, n);
	...
	for (Int_t ign = 0; ign < num; ign++) {
		for (Int_t ifn = 0; ifn < num; ifn++) {
			for (Int_t itn = 0; itn < num; itn++) {
				...
				if (dummyg > gmin && dummyg < gmax && dummyf > fmin
						&& dummyf < fmax && dummyth > thl && dummyth < pi) {
					if (fabs(fabs(z0 / dummyf) - fabs(z1 / dummyg)) < 0.01) {
						if (fabs(
								pow(dummyg, 2) + pow(dummyf, 2)
										- 2 * dummyg * dummyf * cos(dummyth)
										- pow(
												sqrt(
														pow(x0 - x1, 2)
																+ pow(z0 - z1,
