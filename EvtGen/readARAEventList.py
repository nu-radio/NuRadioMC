...
def read_ARA_eventlist(filename):
    ...
    with open(filename, 'r') as fin:
        ...
        for i in xrange(len(data)):
            data[i][3] = np.log10(data[i][3]) * units.eV
            data[i][4] = data[i][4] * units.m
            data[i][6] = hp.get_normalized_angle(0.5 * np.pi - data[i][6])  # convert angles into NuRadioMC coordinate convention
            data[i][7] = hp.get_normalized_angle(0.5 * np.pi - data[i][7])  # convert angles into NuRadioMC coordinate convention
        return data
